...
PRIO_AGG_S          = 15.0  # v2.19: вікно агрегації WS-філів невідомої адреси по (адреса, монета, бік)
PRIO_AGG_MIN_USD    = 0.0   # v2.20 (аудит v2.19, залишкове): пил ТЕЖ накопичується — 100×$200
                            # у вікні 15 с = та сама подія, що $20k одним; вікно й чистка
                            # акумуляторів тримають пам'ять обмеженою
GONE_MAX_S          = 2 * 3600  # v2.20 (аудит №8): пара, яку скан бачить закритою, чекає
                                # підтвердження sweep-ом (філи → подія) не довше за це
_prio_acc           = {}    # (addr, coin, side) -> [сума $, ts початку вікна]
...
def _prio_note(addr, coin, taker_side, t):
    ...
    now = time.time()
    # v2.19 (аудит v2.17, пріоритет 4): $20k одним філом і $20k двадцятьма
    # філами по $1k — та сама подія: нотіонал АГРЕГУЄТЬСЯ по (адреса,
    # монета, бік) у вікні PRIO_AGG_S; поріг перевіряється на сумі
    if notional < PRIO_AGG_MIN_USD:
        return   # дешевий вихід ДО локів: пил не накопичуємо
    with _prio_lock:
        k_agg = (addr, coin, pos_side)
        acc = _prio_acc.get(k_agg)
        if acc is None or now - acc[1] > PRIO_AGG_S:
            acc = [0.0, now]
            _prio_acc[k_agg] = acc
        acc[0] += notional
        notional = acc[0]
        if len(_prio_acc) > 5000:   # чистка застарілих акумуляторів
            for _k in [k for k, v in _prio_acc.items() if now - v[1] > PRIO_AGG_S]:
                _prio_acc.pop(_k, None)
    if notional < PRIO_FLOOR_USD:
        return   # сума ще мала — чекаємо наступних філів вікна
...
def update_watchlist(result, depth_snap, scan_start=0, failed_addrs=None,
    ...
    with watchlist_lock:
        ...
        # v2.20 (аудит v2.19 №8): пара, яку СКАН бачить закритою (нема у знімку
        # гаманця, який скан зчитав успішно), а realtime ще НЕ підтвердив
        # (немає tombstone після старту скану) — НЕ зникає мовчки: лишається
        # у watchlist з міткою _gone_ts, щоб sweep дістав філи й доставив
        # повне закриття стратегіям/алертам (раніше: пара видалялась, sweep її
        ...
        for _wa, _wcoins in list(watchlist.items()):
            ...
            for _wc, _wp in _wcoins.items():
                ...
                if _nw is not None:
                    ...
                    # доставить повне закриття старого боку й вставить новий
                    # (_insert_flipped); інакше реверс-подія старого боку губилась
                _k = f"{_wa}:{_wc}"
                if scan_tombstones.get(_k, 0) >= scan_start > 0:
                    continue   # realtime уже закрив (і заалертив) цю пару
                _g = float(_wp.get("_gone_ts") or 0)
                if _g and _now_wl - _g > GONE_MAX_S:
                    dropped_gone += 1
                    stats["scan_gone_dropped"] = stats.get("scan_gone_dropped", 0) + 1
                    print(f"  [WATCH] {_wc} {_wa[:10]}: закриття не підтверджене sweep-ом за "
                          f"{GONE_MAX_S // 3600} год — пара знята (філів не отримано)")
                    continue
                _cp = dict(_wp)
                if not _g:
                    _cp["_gone_ts"] = _now_wl
                    stats["scan_gone_carried"] = stats.get("scan_gone_carried", 0) + 1
                new_wl.setdefault(_wa, {})[_wc] = _cp   # і при фліпі: старий бік замість нового
                carried_gone += 1
    ...
    print(f"  [WATCH] Watchlist updated: {len(new_wl)} wallets, "
          ...
          + (f" | {carried_gone} закритих чекають підтвердження sweep" if carried_gone else ""))
...
def get_recent_market_fills(addr, coin, since_ms, side=None, with_passive=False):
    """
    ...
    v2.20 (аудит v2.19 №9): with_passive=True → (агресивні, ПАСИВНІ) — мейкерські
    та TWAP-закриття тієї ж позиції (agg=0): вони НЕ тригер стратегій і не
    алерт, але ВХОДЯТЬ в епізод закриття (тривалість/початок), як у settlement.
    """
    ...
    txs_by = {True: {}, False: {}}   # агресивність -> hash -> {sz, cost, ts, oids, dir}
    for f in (fills or []):
        ...
        if not is_close:
            continue
        agg = bool((is_taker or f_liq) and not is_twap)
        if not agg and not with_passive:
            continue
        txs = txs_by[agg]
        oid = f.get("oid")
        h  = f.get("hash")
    ...
    market_txs, passive_txs = [], []
    for _agg, _txs in ((True, txs_by[True]), (False, txs_by[False])):
      for h, t in _txs.items():
        if t["sz"] <= 0: continue
        (market_txs if _agg else passive_txs).append({
            "hash":    h, "agg": int(_agg),
    ...
    if with_passive:
        passive_txs.sort(key=lambda x: x["ts"], reverse=True)
        return market_txs, passive_txs
    return market_txs
...
_ingested = OrderedDict()   # (addr, coin, tx-id) -> {"ts", "ok": {споживачі, що обробили}} (LRU)
_ingested_lock = threading.Lock()
INGEST_LRU = 20000

def _tx_key(f):
    """Ідентичність транзакції для дедупу приймача: hash; системні філи
    (нульовий hash — TWAP-суб-ордери, ліквідації) — час+розмір+ціна."""
    h = str(f.get("hash") or "")
    if not h or h.lower().strip("0x") == "":
        return f"sys:{f.get('ts')}:{f.get('sz')}:{f.get('px')}"
    return h

INGEST_CONSUMERS = ("sim", "fc", "rev", "fol")
INGEST_RETRY_MAX = 50
_fc_done = {}       # (addr, coin) -> ключ останньої tx уже доставленого ПОВНОГО закриття
_ingest_retry = {}  # (addr, coin) -> [tx, що якийсь споживач не підтвердив] (повтор при наступній доставці)

def _ingest_txs(addr, coin, old, mfills, new_size, full_close, det_src, snap_ms, passive=None):
    """v2.19 (аудит v2.17 C): ЄДИНИЙ приймач підтверджених маркет-транзакцій
    для всіх шляхів детекції (WS fast-path / sweep / скан-діф) — дедуп за
    ідентичністю tx (_tx_key, LRU INGEST_LRU), журнал close_batch, SIM,
    епізод (fc_on_txs), реверс, follow, повне закриття.
    v2.20 (аудит v2.19 №9): ІДЕМПОТЕНТНІСТЬ — кожна tx тримає ПІДТВЕРДЖЕННЯ
    успішної обробки КОЖНИМ споживачем (sim/fc/rev/fol): збій одного споживача
    не «з'їдає» tx для нього (наступна доставка повторює лише йому), а
    успішні не дублюються; повне закриття має СТАБІЛЬНИЙ id (адреса, монета,
    остання tx) — повторна доставка тієї самої події (WS↔sweep) не породжує
    другого сигналу/_OUTCOME; passive — мейкерські/TWAP-закриття лише в
    епізод (fc_on_txs), не тригер.
    → (нові tx після дедупу, ev_val, ep_ratio) для алерт-гейта."""
    now = time.time()
    fc_key = (addr, coin)
    with _ingested_lock:
        # рев'ю v2.20: tx, яку якийсь споживач НЕ підтвердив (збій), лежить у
        # retry-списку пари й повторно пред'являється при наступній доставці
    ...
    with _ingested_lock:
        fresh, todo = [], {c: [] for c in INGEST_CONSUMERS}
        for f, k in zip(mfills, keys):
            d = _ingested.get(k)
            if d is None:
                d = {"ts": now, "ok": set(), "busy": set()}
                _ingested[k] = d
                fresh.append(f)
            d.setdefault("busy", set())
            for c in INGEST_CONSUMERS:
                if c not in d["ok"] and c not in d["busy"]:
                    todo[c].append(f)
                    d["busy"].add(c)   # in-flight: паралельна доставка (WS↔скан) не дублює виклик
        while len(_ingested) > INGEST_LRU:
            _ingested.popitem(last=False)
    dup = len(mfills) - len(fresh)
    if dup:
        stats["ingest_dup"] = stats.get("ingest_dup", 0) + dup
    def _release(c, lst, ok):
        """після виклику споживача: успіх — підтвердження; збій — tx у retry-список пари"""
        with _ingested_lock:
            for f in lst:
                d = _ingested.get((addr, coin, _tx_key(f)))
                if d is None:
                    continue
                d.get("busy", set()).discard(c)
                if ok:
                    d["ok"].add(c)
                else:
                    lst_r = _ingest_retry.setdefault(fc_key, [])
                    if _tx_key(f) not in {_tx_key(x) for x in lst_r}:
                        lst_r.append(dict(f, _rt_ts=now))
                    if len(lst_r) > INGEST_RETRY_MAX:
                        del lst_r[:len(lst_r) - INGEST_RETRY_MAX]
            if fc_key in _ingest_retry:
                _done = {_tx_key(f) for f in lst} if ok else set()
                _ingest_retry[fc_key] = [f for f in _ingest_retry[fc_key]
                                         if not (_tx_key(f) in _done
                                                 and all(cc in (_ingested.get((addr, coin, _tx_key(f))) or {}).get("ok", ())
                                                         for cc in INGEST_CONSUMERS))]
                if not _ingest_retry[fc_key]:
                    _ingest_retry.pop(fc_key, None)
    last_key = (max(zip((f.get("ts") or 0 for f in mfills), (k[2] for k in keys)))[1]
                if mfills else f"empty:{snap_ms}")
    fc_event = bool(full_close) and _fc_done.get(fc_key) != last_key
    if not any(todo.values()) and not fc_event:
        if passive:
            try:
                fc_on_txs(addr, coin, old, [], passive=passive)   # пасивні — лише в епізод
            except Exception as _fe:
                print(f"  [FC] passive hook err: {_fe}")
        return [], _grace_val(old), 0
    if fresh:
        stats["ingest_txs"] = stats.get("ingest_txs", 0) + len(fresh)
        stats[f"ingest_{det_src}"] = stats.get(f"ingest_{det_src}", 0) + 1
    if not fresh and not fc_event:
        stats["ingest_retry"] = stats.get("ingest_retry", 0) + 1
    txs = fresh if fresh else list(mfills)
    # аудит-2 №8: первинна подія у журнал — сирі ціни/розміри/час
    # кожної транзакції, знімок позиції, джерело детекції
    if fresh or fc_event:
      try:
        _journal("close_batch", addr=addr, coin=coin, side=old.get("side"),
                 ...
                 new_size=new_size, n_txs=len(txs), n_dup=dup, n_passive=len(passive or ()),
                 truncated=bool(len(txs) > 500),
                 txs=[{"h": str(f.get("hash"))[:18], "px": f["px"],
                       ...
                       "n": f.get("n_orders")}
                      for f in txs[:500]])   # аудит-3: без обрізання на 40
      except Exception as _je:
        print(f"  [JOURNAL] close_batch err: {_je}")
    # ── Симулятор: кожна підтверджена маркет-транзакція (лише ще не оброблені ним) ──
    if todo["sim"]:
        try:
            sim_on_market_txs(addr, coin, old, todo["sim"], new_size, full_close)
            _release("sim", todo["sim"], True)
        except Exception as _se:
            _release("sim", todo["sim"], False)
            print(f"  [SIM] hook err: {_se}")
    # ── Стратегія відкату (п.6): годуємо епізоди (дедуп по hash усередині;
    #    пасивні філи — теж в епізод, аудит №9) ──
    if todo["fc"] or passive:
        try:
            fc_on_txs(addr, coin, old, todo["fc"], passive=passive)
            _release("fc", todo["fc"], True)
        except Exception as _fe:
            _release("fc", todo["fc"], False)
    ...
    ev_val = _grace_val(old)
    ep_ratio = 0
    if full_close:
        with fc_lock:
            _ep_a = fc_episodes.get((addr, coin))
        ev_val = max(ev_val, (_ep_a or {}).get("val") or 0)
        # ratio на СТАРТІ епізоду — в алерт про повне закриття
        # (рев'ю v2.12 №7a: 50%+50% показувало 1.25 замість 2.5)
        ep_ratio = (_ep_a or {}).get("ratio") or 0
    # ── ОБОЛОНКА СТРАТЕГІЙ: реверс + вхід у бік. rev читає
    #    епізод ДО того, як fc_on_full_close його зніме. Повне закриття без
    #    нових tx (гонка WS↔sweep) — подія доставляється ОДИН раз (fc_event)
    txs_rev = todo["rev"] if todo["rev"] else (list(mfills) if fc_event else [])
    if txs_rev:
        try:
            rev_on_close(addr, coin, old, txs_rev, full_close, detect_src=det_src, detect_ms=snap_ms)
            _release("rev", todo["rev"], True)
        except Exception as _re:
            _release("rev", todo["rev"], False)
            print(f"  [REV] hook err: {_re}")
    txs_fol = todo["fol"] if todo["fol"] else (list(mfills) if fc_event else [])
    if txs_fol:
        try:
            follow_on_txs(addr, coin, old, txs_fol, full_close, detect_src=det_src, detect_ms=snap_ms)
            _release("fol", todo["fol"], True)
        except Exception as _fo:
            _release("fol", todo["fol"], False)
    ...
    if full_close:
        _fc_done[fc_key] = last_key
        if len(_fc_done) > 5000:
            for _k in list(_fc_done)[:1000]:
                _fc_done.pop(_k, None)
    return fresh, ev_val, ep_ratio
...
def run_realtime_monitor():
    ...
    def check_wallet_worker(item):
            ...
            for coin, old in coins.items():
                    ...
                    if delta_size <= 0:
                        ...
                        with watchlist_lock:
                            if addr in watchlist and coin in watchlist[addr]:
                                ...
                                _w.pop("_gone_ts", None)   # v2.20: позиція жива — мітка скану знімається
                                _w["val"]   = new_pos["val"]
                ...
                try:
                    mfills, _passive = get_recent_market_fills(addr, coin, since_ms,
                                                               old.get("side"), with_passive=True)
                except RateLimited:
                    stats["rate_limited"] += 1
                    with cycle_rl_lock:
                        cycle_rl[0] += 1
                ...
                if not mfills:
                    if _passive:
                        # v2.20 (аудит №9): пасивні закриття без агресивних — лише
                        # в епізод (початок/тривалість), ДО зсуву курсора
                        try:
                            fc_on_txs(addr, coin, old, [], passive=_passive)
                        except Exception as _pe:
                            print(f"  [FC] passive hook err: {_pe}")
                    stats["fills_empty"] += 1
                    sent_alerts.discard(alert_key)
                ...
                # v2.19 (аудит C): ЄДИНИЙ приймач підтверджених транзакцій —
                # журнал, SIM, епізод, реверс, follow, повне закриття — з
                # дедупом за ідентичністю tx (той самий код у скан-діфі)
                _fresh_txs, _ev_val, _ep_ratio = _ingest_txs(
                    addr, coin, old, mfills, (new_pos["size"] if new_pos else 0.0),
                    full_close, det_src, snap_ms, passive=_passive)
    ...
    for addr, coins in prev.items():
        for coin, old in coins.items():
            ...
            if not mfills:
                if _passive:
                    try:
                        fc_on_txs(addr, coin, old, [], passive=_passive)   # v2.20: лише в епізод
                    except Exception as _pe:
                        print(f"  [FC] passive hook err: {_pe}")
            ...
            for _f in mfills:   # фліп: закритого не більше за позицію
                if ">" in _f.get("dir", "") and _f["sz"] > old["size"]:
                    _f["sz"] = old["size"]
            # v2.19 (аудит C): скан-діф підтвердив дельту філами і пересунув
            # СПІЛЬНИЙ курсор — раніше ці транзакції НЕ доходили до стратегій
            # (пропущений вхід, хибний таймер тиші follow, зламаний початок
            # епізоду). Той самий приймач, що й у realtime; дедуп за tx
            try:
                _ingest_txs(addr, coin, old, mfills, new_pos["size"], False, "scan",
                            int(_now * 1000), passive=_passive)
            except Exception as _ie:
                print(f"  [DIFF] ingest err: {_ie}")
...
SIM_COMMISSION     = 0.0005   # Binance USDⓈ-M taker 0.05% за сторону (VIP0 без BNB-знижки)
SIM_SPREAD         = 0.0002   # базовий спред у сліпаж-моделі
# v2.19 (аудит v2.17, рішення користувача 12.09): біржа ВИКОНАННЯ paper-угод —
# Binance USDⓈ-M. Сигнал — з філів кита на HL, а вхід/вихід/витрати/фандинг —
# по стакану Binance (fapi depth), стрічка aggTrades тієї ж біржі — перевірка.
EXEC_EXCH          = "bn"     # "bn" — Binance; "hl" — старі трекери (до 2.19)
BN_EXEC_LEVELS     = 100      # рівнів стакану для виконання (вага 5)
BOOK_APPLY_MAX_S   = 5.0      # ціна, отримана раніше ніж за стільки с до застосування, — не «зараз»
BOOK_RAW_TTL_S     = 2.0      # сирий стакан (coin, бік, біржа) повторно проходиться різними
                              # кількостями без нового запиту (6 R-трекерів сигналу — 1 запит)
FUND_CACHE_S       = 60.0     # кеш premiumIndex (ставка/момент фандингу) на символ
_raw_book_cache    = {}       # (exch, coin) -> dict(t0, recv, bids, asks, q_ms)
...
def _book_walk(book, side, usd=None, qty=None):
    """Прохід по стороні стакану (список (px, sz)) ринковим ордером:
    usd — ВХІД на суму; qty — ВИХІД фіксованої кількості контрактів,
    куплених на вході (аудит v2.17 E: вихід проходиться на ту саму кількість,
    не на $1000 знову). → dict(px, px_filled, qty, fill_frac, partial,
    last_px): px — ціна для рядка (вхід partial: гірший пройдений рівень;
    вихід partial: заповнена частина за VWAP + незаповнений лишок за
    гіршим видимим рівнем — консервативно, а fill_frac<1 робить рядок
    «частковим виходом», не доказом виконання); px_filled — VWAP
    заповненої частини."""
    by_qty = qty is not None
    target_q = float(qty) if by_qty else None
    target_usd = float(SIM_POSITION_USD if usd is None else usd)
    cost = filled = 0.0
    last_px = None
    for px, sz in book:
        if px <= 0 or sz <= 0:
            continue
        if by_qty:
            take_q = min(target_q - filled, sz)
        else:
            take_q = min(target_usd - cost, px * sz) / px
        if take_q <= 0:
            continue
        cost += take_q * px; filled += take_q; last_px = px
        if (by_qty and filled >= target_q - 1e-12) or (not by_qty and cost >= target_usd - 1e-9):
            break
    if filled <= 0 or not last_px:
        return None
    vwap = cost / filled
    if by_qty:
        fill_frac = min(1.0, filled / target_q) if target_q > 0 else 1.0
        partial = int(fill_frac < 1.0 - 1e-9)
        rest = max(0.0, target_q - filled)
        px_row = (cost + rest * last_px) / target_q if (partial and target_q > 0) else vwap
        q_out = target_q
    else:
        fill_frac = min(1.0, cost / target_usd) if target_usd > 0 else 1.0
        partial = int(fill_frac < 1.0 - 1e-9)
        px_row = last_px if partial else vwap
        q_out = filled
    return {"px": px_row, "px_filled": vwap, "qty": q_out, "fill_frac": fill_frac,
            "partial": partial, "last_px": last_px}

def _book_quote(book_res, bids, asks, q_ms, t0, recv, coin, exch, what):
    """Спільний хвіст книги: вік котирування (біржовий час), мід, momент
    отримання (recv_ms — ЦЕ час виконання, аудит v2.17 I)."""
    q_age = (recv * 1000 - q_ms) / 1000.0 if q_ms else None
    if q_age is not None and abs(q_age) > BOOK_MAX_AGE_S:
        ...
        if stats["book_stale"] in (1, 10, 100, 1000):
            # рев'ю: не мовчати — зсув годинника VPS >5 с від біржі обнулив би
            # УСІ paper-входи без жодного рядка логу
            print(f"  [BOOK] {coin}: котирування {what} старше за {BOOK_MAX_AGE_S:.0f} с "
    ...
    out = dict(book_res)
    out.update({"mid": mid, "bid": best_bid, "ask": best_ask,
                "ts_ms": (q_ms or int(t0 * 1000)), "q_age_s": q_age,
                "req_ms": int((recv - t0) * 1000), "recv_ms": int(recv * 1000),
                "exch": exch})
    return out

def hl_book_exec(coin, side, usd=None, qty=None, min_recv_ms=None):
    """Свіжий стакан HL (l2Book, вага 2) через prio-канал → ВИКОНУВАНА
    ціна ринкового ордера з нашого боку: BUY іде по асках, SELL по бідах,
    VWAP по рівнях (v2.16). v2.19: лише для трекерів, відкритих до 2.19
    (біржа виконання нових — Binance, bn_book_exec); qty — вихід на
    кількість контрактів входу. Повертає dict(px, px_filled, qty, fill_frac,
    mid, bid, ask, ts_ms, recv_ms, req_ms, partial, exch="hl") або None."""
    raw = _raw_book_get("hl", coin, min_recv_ms)
    if raw is None:
        t0 = time.time()
        try:
            ...
            bids = [(float(l["px"]), float(l["sz"])) for l in (lv[0] or [])]
            asks = [(float(l["px"]), float(l["sz"])) for l in (lv[1] or [])]
            if not bids and not asks:
                raise ValueError("порожній стакан")
        except Exception as e:
            stats["book_fail"] = stats.get("book_fail", 0) + 1
            if stats["book_fail"] in (1, 10, 100) or stats["book_fail"] % 1000 == 0:
                print(f"  [BOOK] {coin}: стакан HL недоступний ({stats['book_fail']}): {e}")
            return None
        # аудит v2.16 №14: час котирування — біржовий
        try:
            q_ms = int(data.get("time") or 0)
        except (TypeError, ValueError):
            q_ms = 0
        raw = _raw_book_put("hl", coin, t0, recv, bids, asks, q_ms)
    return _book_from_raw(raw, side, usd, qty, coin, "hl", "l2Book")

def _raw_book_get(exch, coin, min_recv_ms=None):
    """Сирий стакан не старший за BOOK_RAW_TTL_S (рев'ю v2.19): різні
    трекери одного тику виходять різними кількостями — один запит на
    (біржа, монета), прохід по стакану — на кожну кількість.
    v2.20 (аудит v2.19 №1, ПРИЧИННІСТЬ): стакан, ОТРИМАНИЙ до моменту
    рішення (min_recv_ms — час події/рішення, що запитує ціну), не є ціною
    «після сигналу» — кеш його не віддає, буде новий запит."""
    raw = _raw_book_cache.get((exch, coin))
    if raw and time.time() - raw["recv"] <= BOOK_RAW_TTL_S \
            and (not min_recv_ms or raw["recv"] * 1000.0 >= float(min_recv_ms)):
        return raw
    return None

def _raw_book_put(exch, coin, t0, recv, bids, asks, q_ms):
    raw = {"t0": t0, "recv": recv, "bids": bids, "asks": asks, "q_ms": q_ms}
    _raw_book_cache[(exch, coin)] = raw
    if len(_raw_book_cache) > 500:
        for k in [k for k, v in _raw_book_cache.items() if time.time() - v["recv"] > BOOK_RAW_TTL_S]:
            _raw_book_cache.pop(k, None)
    return raw

def _book_from_raw(raw, side, usd, qty, coin, exch, what):
    book = raw["asks"] if side == "BUY" else raw["bids"]
    if not book:
        stats["book_fail"] = stats.get("book_fail", 0) + 1
        return None
    res = _book_walk(book, side, usd=usd, qty=qty)
    if res is None:
        return None
    return _book_quote(res, raw["bids"], raw["asks"], raw["q_ms"], raw["t0"], raw["recv"],
                       coin, exch, what)

def bn_book_exec(coin, side, usd=None, qty=None, min_recv_ms=None):
    """v2.19: біржа ВИКОНАННЯ paper-угод — Binance USDⓈ-M (рішення
    користувача 12.09): свіжий стакан fapi/v1/depth (BN_EXEC_LEVELS рівнів,
    вага 5, напряму — той самий IP, що й цикл глибини) → ціна ринкового
    ордера з нашого боку: BUY по асках, SELL по бідах, VWAP по рівнях.
    usd — вхід на суму; qty — вихід кількості контрактів входу (аудит E).
    Символу нема на Binance (поза всесвітом глибини) — стакану немає,
    paper-угоди на цій монеті НЕ буде (ми не змогли б її виконати).
    Повертає dict(px, px_filled, qty, fill_frac, mid, bid, ask, ts_ms
    (біржовий T), recv_ms (момент отримання = час виконання), req_ms,
    q_age_s, partial, exch="bn") або None."""
    symbol = get_bn_symbol(coin)
    # рев'ю v2.19 №1: всесвіт глибини зберігає НАЗВИ МОНЕТ HL (fetch_all_depth:
    # set(ok_coins)), не символи Binance — звірка за монетою, інакше жоден
    # стакан не запитувався б (book_no_symbol на кожен вхід)
    if _depth_universe[0] and coin not in _depth_universe[0]:
        stats["book_no_symbol"] = stats.get("book_no_symbol", 0) + 1
        return None
    raw = _raw_book_get("bn", coin, min_recv_ms)
    if raw is None:
        if time.time() < _bn_backoff[0]:
            stats["book_fail"] = stats.get("book_fail", 0) + 1
            return None
        ...
        try:
            req = urllib.request.Request(
                f"https://fapi.binance.com/fapi/v1/depth?symbol={symbol}&limit={BN_EXEC_LEVELS}",
                headers={"User-Agent": "Mozilla/5.0"})
            ...
            code = getattr(e, "code", None)
            if code in (418, 429):
                # як у циклі глибини: негайний ретрай веде до 418-бану IP
                try:
                    ra = float((getattr(e, "headers", None) or {}).get("Retry-After") or 0)
                except (TypeError, ValueError, AttributeError):
                    ra = 0.0
            ...
            stats["book_fail"] = stats.get("book_fail", 0) + 1
            if stats["book_fail"] in (1, 10, 100) or stats["book_fail"] % 1000 == 0:
                print(f"  [BOOK] {coin}: стакан Binance недоступний ({stats['book_fail']}): {e}")
            return None
        try:
            q_ms = int(data.get("T") or data.get("E") or 0)
        except (TypeError, ValueError):
            q_ms = 0
        raw = _raw_book_put("bn", coin, t0, recv, bids, asks, q_ms)
    return _book_from_raw(raw, side, usd, qty, coin, "bn", "depth")

_fund_cache = {}   # symbol -> (ts, rate, next_ms)
def bn_funding_info(coin):
    """v2.19: поточна ставка фандингу Binance і момент наступного нарахування
    (premiumIndex, вага 1; кеш FUND_CACHE_S) → (rate, next_ms) або (None, None).
    На HL фандинг щогодини, на Binance — за розкладом символу (8/4/1 год):
    навіть 30-хв позиція може перетнути момент нарахування (аудит v2.17)."""
    symbol = get_bn_symbol(coin)
    c = _fund_cache.get(symbol)
    now = time.time()
    if c and now - c[0] < FUND_CACHE_S:
        return c[1], c[2]
    if now < _bn_backoff[0]:
        return None, None
    try:
        req = urllib.request.Request(
        ...
        with urllib.request.urlopen(req, timeout=4) as r:
            data = json.loads(r.read())
        rate = float(data.get("lastFundingRate"))
        nxt = int(data.get("nextFundingTime") or 0)
    except Exception as e:
        stats["fund_fail"] = stats.get("fund_fail", 0) + 1
        if stats["fund_fail"] in (1, 10, 100):
            print(f"  [BOOK] {coin}: premiumIndex недоступний ({stats['fund_fail']}): {e}")
        return None, None
    _fund_cache[symbol] = (now, rate, nxt)
    return rate, nxt

def _funding_pct(p, exit_ms):
    """Фандинг за час утримання paper-позиції, % від позиції (додатний =
    ми платимо): момент нарахування (fund_next_ms, знятий на вході) у
    (вхід, вихід] → ставка × знак (LONG платить додатну ставку, SHORT —
    отримує). Одне нарахування (утримання ≤120 хв < 4 год); точну суму за
    історією fundingRate рахує settlement. Без даних — 0 і прапорець
    fund_unknown у трекері."""
    nxt, rate = p.get("fund_next_ms"), p.get("fund_rate")
    if not nxt or rate is None:
        return 0.0
    e_ts = p.get("entry_ts") or p.get("open_ts") or 0
    e_ms = int(round(float(e_ts) * 1000))
    if not (e_ms < int(nxt) <= int(exit_ms)):
        return 0.0
    side = p.get("side") or p.get("our_side")
    return float(rate) * 100.0 * (1.0 if side == "LONG" else -1.0)

def _entry_exec_fields(px_entry, pmeta, coin):
    """Поля трекера про виконання входу (v2.19): біржа, кількість контрактів
    на $1000, ставка/момент фандингу (Binance), момент отримання ціни."""
    exch = (pmeta or {}).get("exch") or EXEC_EXCH
    out = {"exch": exch, "entry_qty": (SIM_POSITION_USD / px_entry if px_entry else None),
           "entry_recv_ms": (pmeta or {}).get("recv_ms"),
           "entry_fill_frac": (pmeta or {}).get("fill_frac"),
           "fund_rate": None, "fund_next_ms": None}
    if exch == "bn":
        fr, fn = bn_funding_info(coin)
        out["fund_rate"], out["fund_next_ms"] = fr, fn
        if fr is None:
            out["fund_unknown"] = 1
    return out
...
# ── v2.20 (аудит v2.19 №2): ПОТІК ЦІН BINANCE для Binance-трекерів ──────
# Сигнал (філи кита, BTC-вето, референс епізоду, P0/P1 твапу) — з HL; але
# ПОРОГИ і ВИКОНАННЯ Binance-позиції (тригер R2, TP R8, хвилинні семпли
# кривої, фолбек міда, кап тику) — лише з узгодженого Binance-потоку:
# fapi/v1/ticker/bookTicker на всі символи кожні BN_PX_POLL_S (вага 5).
BN_PX_POLL_S = 3.0
BN_PX_URL    = "https://fapi.binance.com/fapi/v1/ticker/bookTicker"
bn_px_lock   = threading.Lock()
bn_px_hist   = {}   # symbol -> [(ts_send, mid, bid, ask), ...] (останні 50)

def run_bn_px_poller():
    while True:
        t0 = time.time()
        if t0 < _bn_backoff[0]:
            time.sleep(1.0); continue
        try:
            req = urllib.request.Request(BN_PX_URL, headers={"User-Agent": "Mozilla/5.0"})
            ...
            with bn_px_lock:
                for it in data or ():
                    try:
                        sym = it.get("symbol"); bid = float(it.get("bidPrice")); ask = float(it.get("askPrice"))
                    except (TypeError, ValueError, AttributeError):
                        continue
                    if not sym or not (math.isfinite(bid) and math.isfinite(ask)) or bid <= 0 or ask <= 0:
                        continue
                    h = bn_px_hist.setdefault(sym, [])
                    h.append((t0, (bid + ask) / 2.0, bid, ask))
                    if len(h) > 50: del h[:len(h) - 50]
                    n += 1
            stats["bn_px_last_ok"] = time.time(); stats["bn_px_n"] = n
        except Exception as e:
            code = getattr(e, "code", None)
            if code in (418, 429):
                try:
                    ...
                    ra = 0.0
                _bn_backoff[0] = time.time() + max(30.0, ra)
            stats["bn_px_fail"] = stats.get("bn_px_fail", 0) + 1
            if stats["bn_px_fail"] in (1, 10, 100) or stats["bn_px_fail"] % 1000 == 0:
                print(f"  [BNPX] bookTicker недоступний ({stats['bn_px_fail']}): {e}")
        time.sleep(max(0.5, BN_PX_POLL_S - (time.time() - t0)))

def _bn_px_mid_age(coin):
    """Мід Binance (bookTicker) монети, його вік у мс і момент семпла (с);
    (None, None, None) — потоку/символу нема."""
    sym = get_bn_symbol(coin)
    with bn_px_lock:
        h = bn_px_hist.get(sym)
        if not h:
            return None, None, None
        ts, mid = h[-1][0], h[-1][1]
    return mid, int((time.time() - ts) * 1000), ts

def _bn_px_now(coin, max_age=20.0):
    mid, age_ms, _ = _bn_px_mid_age(coin)
    return mid if (mid is not None and (age_ms or 0) <= max_age * 1000) else None

def _mid_age_exch(exch, coin):
    """(mid, age_ms, ts_s) з потоку біржі виконання: "bn" → bookTicker,
    інакше HL allMids (старі трекери до 2.19)."""
    if (exch or EXEC_EXCH) == "bn":
        return _bn_px_mid_age(coin)
    mid, age = _px_mid_age(coin)
    ts = (time.time() - age / 1000.0) if age is not None else None
    return mid, age, ts
...
def _bn_px_age_s():
    with bn_px_lock:
        if not bn_px_hist:
            return None
        ts = max(h[-1][0] for h in bn_px_hist.values() if h)
    return round(time.time() - ts, 1)

def _tr_exch(p):
    """Біржа виконання трекера — записана при вході (v2.19); трекер без
    поля (до 2.19) виконувався на HL — і виходить/семплюється там само."""
    return p.get("exch") or ("hl" if _vt(p.get("algo_v")) < (2, 19) else EXEC_EXCH)

def _tr_px_now(p, max_age=30.0):
    """v2.20 (аудит v2.19 №2): поточна ціна ДЛЯ ТРЕКЕРА — з потоку його
    біржі виконання: тригер R2, TP R8, хвилинні семпли Binance-позиції
    рахуються по Binance, не по HL-міду з Binance-входом."""
    return (_bn_px_now(p["coin"], max_age) if _tr_exch(p) == "bn"
            else _px_now(p["coin"], max_age=max_age))

def _paper_px(coin, side, whale_px=None, max_mid_age_ms=20_000, strict=False,
              qty=None, exch=None, min_recv_ms=None):
    ...
    гірша = вища, SELL: нижча. Стакан, що не вмістив обсяг (partial), —
    не доказ повного виконання: у strict = нема стакану, інакше src
    'book_partial' (meta.fill_frac — заповнена частка; рядок стає
    «частковим виходом», не підтвердженою ціною — аудит v2.17 E).
    v2.19: exch — біржа виконання ("bn" Binance для нових трекерів, "hl" —
    старі), qty — вихід на кількість контрактів входу; meta.recv_ms —
    момент отримання ціни (= час виконання, аудит I). Вік міда міряється
    ПІСЛЯ запиту стакану.
    v2.20 (аудит v2.19 №1/№2): min_recv_ms — мінімальний момент ОТРИМАННЯ
    котирування (час рішення): стакан чи мід, отримані раніше, не є ціною
    після сигналу (кеш їх не віддає; мід старіший за рішення → 'none');
    мід-фолбек — з потоку ТІЄЇ Ж біржі (Binance bookTicker для "bn"), а
    не HL allMids з позначкою exch="bn"."""
    exch = exch or EXEC_EXCH
    meta = {"mid": None, "px_age_ms": None, "whale_px": whale_px,
            "book_ms": None, "partial": 0, "quote_ts_ms": None, "exch": exch,
            "recv_ms": None, "fill_frac": None, "px_filled": None, "qty": None,
            "min_recv_ms": min_recv_ms}
    bx = (bn_book_exec(coin, side, qty=qty, min_recv_ms=min_recv_ms) if exch == "bn"
          else hl_book_exec(coin, side, qty=qty, min_recv_ms=min_recv_ms))
    mid, age_ms, mid_ts = _mid_age_exch(exch, coin)   # ПІСЛЯ запиту: вік на момент входу
    ...
    if bx:
        ...
        meta["recv_ms"] = bx.get("recv_ms"); meta["fill_frac"] = bx.get("fill_frac")
        meta["px_filled"] = bx.get("px_filled"); meta["qty"] = bx.get("qty")
    ...
    meta["recv_ms"] = int(time.time() * 1000)   # мід/семпл — «зараз» після запиту
    if strict:
        return None, "no_book", meta
    if mid is None or (age_ms or 0) > max_mid_age_ms:
        return None, "none", meta
    if min_recv_ms and mid_ts and mid_ts * 1000.0 < float(min_recv_ms):
        # семпл міда знято ДО рішення — не «ціна після сигналу» (аудит №1)
        stats["px_pre_decision"] = stats.get("px_pre_decision", 0) + 1
        return None, "none", meta
    meta["recv_ms"] = int(mid_ts * 1000) if mid_ts else meta["recv_ms"]   # момент семпла — час ціни
    px, src = mid, "mid"
    if whale_px:
...
def _pending_rev_rows(st, csv_keys):
    """v2.19 (аудит v2.17 J): R-угода закривається на m30 (вирішений вихід),
    а рядок пишеться лише на m60 (крива) — до того статистика завершених
    результатів відставала. Закриті, ще не записані трекери стратегії →
    pending-рядки (net зі стакану виходу, сигнал/ціна ще не перевірені:
    sig_ok/px_ok=None — у журналі paper і списку, не в заголовку)."""
    out = []
    with strat2_lock:
        items = [(k, dict(p)) for k, p in rev_open.items()
                 if p.get("strategy") == st and not p.get("done")
                 and p.get("exit_reason") and p.get("exit_px") and p.get("entry_px")]
    for k, p in items:
        if _tracker_csv_key("rev", k, p) in csv_keys:
            continue   # рядок уже у CSV (аварійний повтор)
        if not _v_ok(p.get("algo_v"), STRAT_SINCE.get(st, DATA_ALGO_V)):
            continue
        try:
            gx = (float(p["exit_px"]) / float(p["entry_px"]) - 1.0) * 100.0
            if p.get("side") == "SHORT": gx = -gx
            net = gx - _leg_costs(_p_costs(p), p.get("entry_src"), p.get("exit_src")) \
                  - float(p.get("funding_pct") or 0.0)
        except (TypeError, ValueError, ZeroDivisionError):
            continue
        reason = p.get("exit_reason") or ""
        _ff = p.get("exit_fill_frac")
        out.append({"date": _dt(p.get("detect_ts")), "coin": p.get("coin"), "side": p.get("side"),
                    "net30": net, "net_live": net, "net_tape": None, "settled": 0, "flags": "",
                    "status": "pending", "mismatch": 0, "no_price": 0,
                    "peak": (p.get("peak") if (p.get("peak") or -999) > -999 else None),
                    "trough": (p.get("trough") if (p.get("trough") or 999) < 999 else None),
                    "move": p.get("move"), "dur": p.get("dur"), "ratio": p.get("ratio"),
                    "shtanga": p.get("shtanga", 0), "vault": p.get("vault", 0), "hour": p.get("hour"),
                    "btc": p.get("btc_move"), "sum_usd": p.get("sum_usd"), "p3u": None, "p3d": None,
                    "late_exit": (1 if reason == "timer_late" else 0), "late": (1 if reason == "timer_late" else 0),
                    "stale": 0, "exit_reason": reason, "exit_min": p.get("exit_min"),
                    "grace": p.get("grace"), "bucket": p.get("dump_bucket"), "dump_dur": p.get("dump_dur"),
                    "dump_move": p.get("dump_move"), "dump_move_live": p.get("dump_move"),
                    "bucket_live": p.get("dump_bucket"), "ep_mismatch": 0, "full_close": None,
                    "lag": p.get("lag_s"), "entry_src": p.get("entry_src") or "",
                    "exit_src": p.get("exit_src") or "", "detect_src": p.get("detect_src") or "",
                    "adm": 0, "sig_ok": None, "sig_why": "pending", "px_ok": None,
                    "exit_ok": (0 if reason == "timer_late" else 1), "exit_why": "",
                    "exit_partial": int((_ff is not None and _ff < 1.0 - 1e-9)
                                        or str(p.get("exit_src") or "").startswith("book_partial")), "fill_frac": _ff,
                    "funding": p.get("funding_pct"), "exch": p.get("exch") or "",
                    "exec_delay": p.get("exec_delay_s"), "net_tp_mkt": None, "net_tp_lim": None,
                    "entered": 1, "ts": p.get("detect_ts"), "pending_row": 1,
                    "wallet": (p.get("addr") or "")[:10], "_ct": None, "_cl": None})
    return out
...
def _fc_rebuild(ep_txs, old, seen, prev=None):
    ...
    ep = {"first_ts": t0["ts"], "first_px": (t0.get("px_first") or t0["px"]),
          ...
          "txs": [{k: t.get(k) for k in ("hash", "ts", "px", "px_first", "px_last",
                                          "sz", "sp", "dir", "liq", "agg")} for t in ep_txs]}
    for t in ep_txs:
        if int(t.get("agg", 1)) == 0:
            continue   # v2.20: база R7 «одним пострілом» — лише агресивна tx
        if t["sz"] > ep["max_sz"]:
...
def fc_on_txs(addr, coin, old, mfills, passive=None):
    ...
    if not FC_ENABLED or not (mfills or passive): return
    key = (addr, coin)
    # v2.20 (аудит v2.19 №9): епізод будується на ВСІХ закриттях позиції —
    # агресивних (agg=1: тригер стратегій) і пасивних (мейкер/TWAP, agg=0:
    # лише тривалість/початок), як у settlement (close_episode над усіма
    # філами) — інакше live бачив 60 с там, де settlement бачить 360
    _all = [dict(t, agg=(1 if t.get("agg") is None else int(t.get("agg")))) for t in (mfills or ())]
    _all += [dict(t, agg=0) for t in (passive or ())]
    with fc_lock:
        ep = fc_episodes.get(key)
        seen = ep["seen"] if ep else set()
        new = sorted([t for t in _all if t.get("hash") not in seen],
                     key=lambda t: t.get("ts", 0))
        if not new: return
        # аудит v2.17 D: активний FC-трек (fc_positions — стара стрічка
        # fc_trades) НЕ керує епізодом: раніше при відкритій FC-угоді нові
        # tx лише дописувались (без перебудови бази R7/тривалості), і той
        # самий потік одним чи трьома батчами давав різний допуск R7. Трек
        # тримає СВІЙ знімок (sum_usd/duration у pos), епізод перебудовується
        ...
        if _close_episode is not None:
            # v2.20: у потоці ВСІ закриття (і мейкерські) — абсолютний допуск
            # «позиція не виросла», як у settlement (grow_only лише для
            # епізодів зі старого state без пасивних філів)
            _grow_only = not any(int(t.get("agg", 1)) == 0 for t in txs_all)
            _first, _last, ep_f = _close_episode(fills, coin, t_last, True, grow_only=_grow_only)
...
# v2.18 (аудит пошуку швидких гаманців №8/№10): профіль більше НЕ залежить
# від поточної глибини (nr-гілка = основна), тож F5 ≡ F7 за відбором —
# F5 отримує ФІКСОВАНИЙ вихід 60 с (як F6: різниця F5–F6 = лише профіль),
# F7 лишає таймер профілю (різниця F5–F7 = лише таймер); F10 — F4 з
# фіксованим 60 с (різниця F1–F10 = лише профіль)
F10_NAME         = "F10_розумний_60"
...
STRAT_SINCE = {
    ...
    # v2.20: R2/R8 — тригер/TP тепер по Binance-потоку (до 2.20 — HL-мід проти
    # Binance-входу: інша семантика перетину), рядки 2.17–2.19 не порівнянні
    "R1_загальний": "2.17", "R2_breakout": "2.20", "R3_великі": "2.17",
    "R4_великий": "2.17", "R5_дуже": "2.17", "R6_волт": "2.17",
    "R7_одним": "2.17", R8_NAME: "2.20",
    "F1_1хв": "2.10", "F2_2хв": "2.10", "F3_3хв": "2.10",
    "F6_1хв_перший": "2.10",
    # v2.18: профіль v10 (життєві цикли, без ratio-гейта, uncertain), F5 —
    # фіксований вихід 60 с, F10 новий — рядки з профілем до 2.18 не порівнянні
    "F4_розумний": "2.18", "F5_перший": "2.18", "F7_без_ратіо": "2.18",
    F10_NAME: "2.18",
    ...
    F8_NAME: "2.12", F9_NAME: "2.18", T1_NAME: "2.15", T2_NAME: "2.15",
...
PROFILE_ALGO_V   = 11     # версія алгоритму профілів: старі записи без
...
EVAL_SINCE       = "2026-09-13"   # v2.20 (аудит v2.19, рекомендація 6): правила кандидатних
                                  # стратегій ЗАФІКСОВАНО; результат нового періоду (з цієї
                                  # дати, локальний час) показується ОКРЕМО від того, на якому
                                  # обирались пороги (блок eval у картці/зрізі)
...
DATA_ALGO_V      = "2.20" # версія логіки збору: трекер отримує її при
...
STRAT2_TITLES = {
    ...
    "F5_перший":    "За китом · швидкі гаманці · перший постріл · вихід 60 с",
    "F10_розумний_60": "За китом · швидкі гаманці · вихід 60 с",
...
STRAT2_DESC = {
    ...
    "F4_розумний":  "Разом з китом, але лише за гаманцями, які за 3 місяці "
                    "мали ≥5 ПІДТВЕРДЖЕНО швидких великих розвантажень "
                    "(≥$100k; один життєвий цикл позиції = один епізод, "
                    "початок поза історією = «невизначений») і ≥70% швидких "
                    "серед усіх (невизначені рахуються як повільні); ratio "
                    "≥2 — на момент сигналу, не в історії; вихід 2×їхня "
                    "пауза між шматками; профіль зі статусом «невизначений» "
                    "— окрема позначка, поза заголовком",
    "F5_перший":    "Швидкі гаманці, вхід лише на ПЕРШІЙ транзакції пари "
                    "гаманець:монета (або після паузи ≥1 год), вихід "
                    "ФІКСОВАНИЙ 60 с — порівняння з F6 ізолює профіль, з F7 "
                    "— таймер",
    "F10_розумний_60": "Як F4 (кожна достатня транзакція швидкого гаманця), "
                    "але вихід фіксований 60 с — порівняння з F1 ізолює "
                    "профіль",
    ...
    "F7_без_ратіо": "Швидкі гаманці · перший постріл · вихід 2×пауза профілю "
                    "(з v2.18 профіль без ratio-гейта в історії для всіх: "
                    "F7 = F5 з таймером профілю)",
...
REV_HEADERS = (["sig_id", "strategy", "date", "coin", "our_side", "whale_addr",
                ...
                # v2.19: виконання на Binance (біржа, кількість, частковий
                # вихід, фандинг, затримка застосування ціни)
                "exch", "entry_qty", "exit_fill_frac", "funding_pct", "exec_delay_s",
...
FOLLOW_HEADERS = ["date_open", "date_close", "strategy", "coin", "our_side",
                  ...
                  "whale_px", "exit_src", "exit_px_mid", "grace",
                  # v2.18: статус профілю на момент входу (ok / uncertain /
                  # pending / no / none), невизначені епізоди, нижня межа
                  # Вілсона, придатність до follow (продовження після входу,
                  # частка one-shot, медіана обсягу після входу)
                  "prof_status", "prof_n_uncertain", "prof_fast_lb95", "prof_cont_pct",
                  "prof_one_shot_pct", "prof_after_pct_med",
                  # v2.19: виконання на Binance
                  "exch", "entry_qty", "exit_fill_frac", "funding_pct", "exec_delay_s",
                "decision_ms", "detect_ms", "exit_decision_ms", "exit_px_filled", "trig_hash", "eol"]
...
def rev_on_close(addr, coin, old, mfills, full_close, detect_src="sweep", detect_ms=None):
    ...
    # v2.20 (аудит v2.19 №1): рішення = старт обробника; котирування, отримане
    # раніше (кеш стакану, семпл міда), не є ціною ПІСЛЯ сигналу
    decision_ms = int(now * 1000)
    px_entry, entry_src, pmeta = _paper_px(coin, "BUY" if side == "LONG" else "SELL",
                                           whale_px=last_px, strict=True, min_recv_ms=decision_ms)
    no_book = not px_entry
    if no_book:
        stats["rev_no_book"] = stats.get("rev_no_book", 0) + 1
        # тіньова outcome-стрічка від міда ТІЄЇ Ж біржі (аудит №2), знятого не
        # раніше за ПОДІЮ (останній філ кита): семпл поллера завжди старший за
        # рішення, але не має бути старшим за сигнал (причинність, аудит №1)
        _mid0, _age0, _mts0 = _mid_age_exch(pmeta.get("exch") or EXEC_EXCH, coin)
        _ev_ms = int(_txs[-1].get("ts") or 0)
        if _mid0 is None or (_age0 or 0) > 20_000 or (_mts0 and _ev_ms and _mts0 * 1000.0 < _ev_ms):
            stats["rev_no_px"] = stats.get("rev_no_px", 0) + 1
            return
        px_entry, entry_src = _mid0, "mid"
        pmeta = dict(pmeta); pmeta["recv_ms"] = int(_mts0 * 1000) if _mts0 else decision_ms
    # аудит v2.17 I: час виконання = момент ОТРИМАННЯ ціни (запит стакану
    # міг тривати секунди), не «зараз» після нього
    t_entry = (pmeta.get("recv_ms") / 1000.0) if pmeta.get("recv_ms") else time.time()
    px_now = pmeta.get("book_mid") or pmeta.get("mid") or px_entry
    _xf = _entry_exec_fields(px_entry, pmeta, coin) if not no_book else {"exch": pmeta.get("exch") or EXEC_EXCH}
    ...
    if lag_s is not None and lag_s < 0:
        # аудит v2.19 №1: НИЖНЯ межа лага — ціна, отримана до філа кита,
        # порушує причинність (зсув годинника / стара котировка): входу немає
        stats["rev_neg_lag"] = stats.get("rev_neg_lag", 0) + 1
        _journal("rev_skip", addr=addr, coin=coin, why="neg_lag", fill_ts_ms=fill_ts_ms,
                 lag_s=round(lag_s, 3), recv_ms=pmeta.get("recv_ms"), decision_ms=decision_ms)
        return
    ...
    # v2.20 (аудит v2.19 №9): СТАБІЛЬНИЙ id сигналу — (монета, адреса, час і
    # hash останньої tx батчу): повторна доставка тієї самої події дає той
    # самий ключ (_OUTCOME перезаписується, не дублюється); різні батчі
    ...
    base_pos = {"sig_id": sig_id, "coin": coin,
                ...
                "whale_px": last_px,
                # v2.20: окремі мітки часу — подія (fill_ts_ms), виявлення,
                # рішення; hash останньої транзакції батча (тригер сигналу)
                "detect_ms": detect_ms, "decision_ms": decision_ms,
                "trig_hash": str(_txs[-1].get("hash") or ""),
                "samples": [], "peak": -999.0, "trough": 999.0}
    base_pos.update(_xf)   # v2.19: біржа, кількість контрактів, фандинг
    opened = []
    with strat2_lock:
        ...
        if not stale:
            # рев'ю v2.19 №8: запізніла детекція (лаг > REV_MAX_FILL_AGE_S —
            # скан-діф віддає філи до години давності) не породжує тіньовий
            # рядок: «після закриття» від ціни, що вже не та, — не спостереження
            pos = dict(base_pos)
...
def _rev_trade_closed(p, now):
    ...
    if not et:
        return False
    if _vt(p.get("algo_v")) >= (2, 16):
        # аудит v2.17 J: трекер ≥2.16 має ВИРІШЕНИЙ вихід (exit_reason: TP /
        # таймер / timer_late / no_price — не пізніше 33:35) — монета зайнята,
        # поки виходу немає; довжина семплів чи час без виконаного виходу
        # її не звільняють (вихід ще може повторюватись)
        return False
...
def _rev_extra(p):
    ...
    return [_ms(p.get("entry_ts")), (p.get("fill_ts_ms") or ""),
            ...
            p.get("exit_src", ""), _rnd(p.get("tp_px"), 8)] + _exec_extra(p)

def _exec_extra(p):
    """v2.19: колонки виконання у рядках R/F/TWAP — біржа, кількість
    контрактів входу, заповнена частка виходу, фандинг (%), затримка між
    отриманням ціни і застосуванням (с). v2.20 (аудит v2.19 №1/№5): окремі
    мітки часу — виявлення (detect_ms: знімок/WS-подія), рішення входу
    (decision_ms: старт обробника), рішення виходу (exit_decision_ms: фаза 1
    тику); VWAP заповненої частини виходу (exit_px_filled) і hash тригерної
    транзакції (trig_hash — settlement звіряє САМЕ її, аудит №3)."""
    _ms_ = lambda v: (int(v) if isinstance(v, (int, float)) and v else "")
    return [p.get("exch", ""), _rnd(p.get("entry_qty"), 8),
            _rnd(p.get("exit_fill_frac"), 4), _rnd(p.get("funding_pct"), 5),
            _rnd(p.get("exec_delay_s"), 3),
            _ms_(p.get("decision_ms")), _ms_(p.get("detect_ms")), _ms_(p.get("exit_decision_ms")),
            _rnd(p.get("exit_px_filled"), 8), (p.get("trig_hash") or "")]
...
def _fol_row(p, fid, close_ts, exit_px, reason, g):
    ...
    fund = float(p.get("funding_pct") or 0.0)   # v2.19: фандинг Binance за утримання
    _bm = p.get("btc_move")
    pr = p.get("prof") or {}
    return [_dt(p["open_ts"]), _dt(close_ts),
            ...
            round(costs, 4), round(g - costs_eff - fund, 4),
            ...
            # v2.18
            p.get("prof_status", ""), pr.get("n_uncertain", ""),
            (pr.get("fast_lb95") if pr.get("fast_lb95") is not None else ""),
            (pr.get("cont_pct") if pr.get("cont_pct") is not None else ""),
            (pr.get("one_shot_pct") if pr.get("one_shot_pct") is not None else ""),
            (pr.get("after_pct_med") if pr.get("after_pct_med") is not None else "")] \
           + _exec_extra(p)

# ── ВХІД У БІК ТИСКУ ────────────────────────────────────
def follow_on_txs(addr, coin, old, mfills, full_close, detect_src="sweep", detect_ms=None):
    if not STRAT2_ENABLED or not mfills: return
    key = f"{addr}:{coin}"
    now = time.time()
    # v2.18 (аудит пошуку швидких гаманців №1): історія гаманця замовляється
    # на КОЖНОМУ підтвердженому закритті — до ранніх відмов (повне закриття,
    # старий філ, малий шматок, без стакану): це причини не входити, а не
    # причини не вивчати гаманець (TTL/версія гейтять усередині, черга —
    # стеля PROFILE_QUEUE_MAX). Повні закриття теж поповнюють дослідницький
    # набір
    ...
    # v2.20 (аудит v2.19 №1): котирування — не раніше за рішення (старт обробника)
    decision_ms = int(now * 1000)
    px, entry_src, pmeta = _paper_px(coin, "SELL" if our == "SHORT" else "BUY",
                                     whale_px=tx["px"], strict=True, min_recv_ms=decision_ms)
    if not px:
        stats["follow_no_book"] = stats.get("follow_no_book", 0) + 1
        _journal("follow_skip", addr=addr, coin=coin, why="no_book",
                 fill_ts_ms=fill_ts_ms, lag_s=lag_s, detect_src=detect_src)
        return
    # аудит v2.17 I: час виконання = момент ОТРИМАННЯ ціни зі стакану
    t_entry = (pmeta.get("recv_ms") / 1000.0) if pmeta.get("recv_ms") else time.time()
    ...
    if lag_s is not None and lag_s > FOLLOW_MAX_FILL_AGE_S:
        ...
        return
    if lag_s is not None and lag_s < 0:
        # аудит v2.19 №1: нижня межа лага — ціна ДО тригерного філа = порушення
        # причинності (кеш/зсув годинника); входу немає, подія у журналі
        stats["follow_neg_lag"] = stats.get("follow_neg_lag", 0) + 1
        _journal("follow_skip", addr=addr, coin=coin, why="neg_lag", fill_ts_ms=fill_ts_ms,
                 lag_s=round(lag_s, 3), recv_ms=pmeta.get("recv_ms"), decision_ms=decision_ms,
                 detect_src=detect_src)
        return
    vault = is_vault(addr)
    ...
    base = {"key": key, "coin": coin, "our_side": our, "addr": addr,
            ...
            # v2.20: мітки часу й ідентичність тригерної транзакції (аудит №1/№3)
            "detect_ms": detect_ms, "decision_ms": decision_ms,
            "trig_hash": str(tx.get("hash") or ""), "trig_dir": tx.get("dir") or "",
            "trig_sz": tx.get("sz"), "trig_sp": tx.get("sp"),
    ...
    base.update(_entry_exec_fields(px, pmeta, coin))   # v2.19: біржа, кількість, фандинг
    ...
    # v2.18 (№3, м'яка редакція): статус профілю — ok (підтверджено:
    # невизначені епізоди рахуються як повільні) або uncertain (пройшло б,
    # якби невизначені були швидкими): ОБИДВА відкривають paper-угоди, але
    # рядок несе prof_status, і в заголовок картки йдуть лише ok — так
    # група «невизначених» перевіряється статистикою, а не викидається
    _fresh_prof = (prof_valid and not prof.get("truncated")
                   and now - prof.get("fetched", 0) < PROFILE_HARD_TTL_S)
    _pst = (prof.get("status") or ("ok" if prof.get("ok") else "no")) if prof_valid else "none"
    f4_ok = bool(_fresh_prof and _pst in ("ok", "uncertain"))
    # F7/F9: з v2.18 профіль без ratio-гейта в історії — та сама кваліфікація
    nr_prof = (prof.get("nr") or prof) if prof_valid else {}
    f7_ok = f4_ok
    # статус профілю у рядку — та сама шкала, що у _profile_status / API
    # (unknown-none → pending: історію щойно замовлено вище)
    if prof is None or prof.get("v") != PROFILE_ALGO_V:
        prof_status = "pending"
    elif prof.get("err"):
        prof_status = "err"
    elif not _fresh_prof:
        prof_status = "pending"      # діра в історії / старший за жорстку стелю — на рефреш
    else:
        prof_status = _pst
    base["prof_status"] = prof_status
    if prof_valid:
        base["prof"] = {"n_fast": prof.get("n_fast", 0),
                        "n_slow": prof.get("n_slow", 0),
                        "n_uncertain": prof.get("n_uncertain", 0),
                        "fast_pct": prof.get("fast_pct"),
                        "fast_lb95": prof.get("fast_lb95"),
                        "unload_med_s": prof.get("unload_med_s"),
                        "unload_mean_s": prof.get("unload_mean_s"),
                        "window_d": prof.get("window_d"),
                        "cont_pct": prof.get("cont_pct"),
                        "one_shot_pct": prof.get("one_shot_pct"),
                        "after_pct_med": prof.get("after_pct_med")}
    with strat2_lock:
        ...
        _prof_strats = []
        if f4_ok:
            # таймер профілю: 2×середня (по епізодах) пауза між агресивними
            # шматками, кламп 60–300 с (v2.18: по ВСІХ завершених епізодах)
            gap2 = 2.0 * float(prof.get("avg_gap_s", 0) or 0)
            timer4 = min(max(gap2, F4_CLAMP[0]), F4_CLAMP[1])
            _prof_strats += [(F4_NAME, timer4, True, None),
                             (F10_NAME, F_FIXED_TIMER_S, True, None),        # F4 · вихід 60 с
                             (F5_NAME, F_FIXED_TIMER_S, first_shot, None),   # перший постріл · 60 с
                             (F7_NAME, timer4, first_shot, None)]            # перший постріл · таймер профілю
            # F9 (ТЗ 08.09 п.4): ≥90% ПІДТВЕРДЖЕНО швидких — по точних
            # лічильниках, невизначені рахуються як повільні (v2.18)
            _nf = int(prof.get("n_fast") or 0)
            _nb = int(prof.get("n_big") or 0)
            _f9 = (_nf >= F4_MIN_EPISODES and _nb > 0
                   and 100.0 * _nf >= F9_MIN_FAST_PCT * _nb)
            _prof_strats.append((F9_NAME, timer4, first_shot and _f9, None))
        ...
        if not fo_busy:
            ...
            rev_open[fo_id] = {
                ...
                "samples": [], "peak": -999.0, "trough": 999.0}
    # історію замовлено на початку функції (v2.18) — тут лише лог
...
def _profile_status(addr):
    """Статус профілю гаманця (v2.18, аудит №1): unknown — історії не
    запитували; pending — у черзі / стара версія алгоритму / застарів;
    err — історія не отрималась; ok — підтверджено швидкий; uncertain —
    кваліфікація невизначена (початок частини епізодів поза історією);
    no — не швидкий."""
    a = (addr or "").lower()
    with strat2_lock:
        prof = wallet_profiles.get(a)
        fetching = a in profiles_fetching
    if prof is None:
        return "pending" if fetching else "unknown"
    if prof.get("v") != PROFILE_ALGO_V or fetching:
        return "pending"
    if prof.get("err"):
        return "err"
    if prof.get("truncated") or time.time() - prof.get("fetched", 0) >= PROFILE_HARD_TTL_S:
        return "pending"
    return prof.get("status") or ("ok" if prof.get("ok") else "no")

PROFILE_REFRESH_EVERY_S = 60     # v2.18: фонова черга профілів — крок циклу
PROFILE_REFRESH_BATCH   = 3      # адрес за крок (≈20 с історії кожна → ~180/год)

def _profile_refresh_pick(now=None):
    """Кого профілювати наступними (аудит №1/пріоритет 2): спершу гаманці
    watchlist БЕЗ профілю чи зі старою версією алгоритму, потім найстаріші
    профілі watchlist (TTL), потім відомі швидкі/невизначені адреси поза
    watchlist (лишаються у пошуку і без позиції). → список адрес."""
    now = now or time.time()
    with watchlist_lock:
        wl = [a.lower() for a in watchlist.keys()]
    with strat2_lock:
        profs = dict(wallet_profiles)
        fetching = set(profiles_fetching)
        retry = dict(profile_retry_at)
    def _need(a):
        if a in fetching or now < retry.get(a, 0):
            return None
        p = profs.get(a)
        if p is None:
            return (0, 0.0)
        if p.get("v") != PROFILE_ALGO_V:
            return (1, 0.0)
        age = now - p.get("fetched", 0)
        soft = p.get("err") or p.get("truncated")
        if age >= (PROFILE_ERR_TTL_S if soft else PROFILE_TTL_S):
            return (2, -age)
        return None
    cands = []
    for a in wl:
        r = _need(a)
        if r is not None:
            cands.append((r[0], r[1], a))
    wl_set = set(wl)
    for a, p in profs.items():
        if a in wl_set or not isinstance(p, dict):
            continue
        if (p.get("status") in ("ok", "uncertain") or p.get("ok")):
            r = _need(a)
            if r is not None:
            ...
            # v2.20 (аудит v2.19, залишкове): профіль зі збоєм (err) поза
            # watchlist — повторити після PROFILE_ERR_TTL_S, а не назавжди
            r = _need(a)
            if r is not None:
                cands.append((7, r[1], a))
    # v2.20: адреси, чий запит відклали (429/бюджет) і профілю ще НЕМА, а з
    # watchlist вони вже випали — раніше не потрапляли до жодного списку
    for a, t_ in retry.items():
        if a in wl_set or a in profs or a in fetching or now < t_:
            continue
        cands.append((8, 0.0, a))
    cands.sort()
    return [a for _, _, a in cands[:PROFILE_REFRESH_BATCH]]

def run_profile_refresher():
    """Фонова черга профілів (v2.18): раз на хвилину до PROFILE_REFRESH_BATCH
    адрес з _profile_refresh_pick → _profile_request. Раніше історія
    замовлялась лише на першому придатному сигналі — після рішення про вхід;
    невідомий гаманець із повним закриттям одним ордером не вивчався взагалі."""
    time.sleep(20)
    while True:
        try:
            for a in _profile_refresh_pick():
                _profile_request(a)
                stats["profile_refresh_req"] = stats.get("profile_refresh_req", 0) + 1
        except Exception as e:
            stats["profile_refresh_err"] = stats.get("profile_refresh_err", 0) + 1
            if stats["profile_refresh_err"] in (1, 10, 100):
                print(f"  [F4] рефрешер профілів: {e!r}")
        time.sleep(PROFILE_REFRESH_EVERY_S)
...
PROFILE_ENTRY_LAT_MS = 5000   # v2.18: «реально доступний вхід» — через 5 с після сигналу
PROFILE_ZERO_TOL = 1e-6       # v2.18: «фактичний нуль» позиції — залишок ≤ 1e-6 від старту tx

def _wilson_lb(k, n, z=1.96):
    """Нижня межа 95% інтервалу Вілсона для частки k/n (0..1); None без n."""
    if not n:
        return None
    ph = k / n
    den = 1.0 + z * z / n
    centre = ph + z * z / (2.0 * n)
    adj = z * math.sqrt(ph * (1.0 - ph) / n + z * z / (4.0 * n * n))
    return max(0.0, (centre - adj) / den)

def _profile_txs(fills):
    """Філи гаманця → транзакції (ордери) у хронології, з відкриттями і
    закриттями. Повертає ({coin: [tx, …]}, bad_rows). tx: dict(t, px, sz, sp,
    kind ∈ open/close, aggr, side, seq). Фліп «Long > Short» = закриття
    старого боку (≤ позиції) + відкриття нового. Структурні поля
    обов'язкові (аудит v2.4 п.3); системні філи (TWAP-суб-ордери,
    ліквідації, ADL — нульовий hash) групуються по oid/tid."""
    txs = {}
    bad = 0
    for seq, f in enumerate(fills):
        if not all(k in f for k in ("dir", "crossed", "px", "sz", "time",
                                    "startPosition", "coin", "hash")):
            bad += 1
            continue
        if not f.get("hash"):
            bad += 1
        ...
        is_flip = ">" in d
        is_close = d.startswith("Close") or f_liq
        if not (is_open or is_flip or is_close):
            continue
        try:
            ...
            bad += 1
            continue
        if not (math.isfinite(px) and math.isfinite(sz) and math.isfinite(sp)):
            bad += 1
            continue
        if t <= 0 or px <= 0 or sz <= 0:
            bad += 1
            continue
        if d.startswith("Close Long") or d.startswith("Long >") or d.startswith("Open Long"):
            side = "LONG"
        elif d.startswith("Close Short") or d.startswith("Short >") or d.startswith("Open Short"):
            side = "SHORT"
        else:
        ...
        if h.lower().strip("0x") == "" or f.get("twapId") is not None or f_liq:
            k = (f.get("coin", "?"), "sys",
                 f.get("oid") if f.get("oid") is not None else f.get("tid", t))
        else:
            k = (f.get("coin", "?"), h)
...
def _profile_lifecycles(txs):
    """Транзакції однієї монети у хронології → життєві цикли позиції
    (аудит v2.18 №2/№4): цикл починається ВІДКРИТТЯМ з нуля (start_known)
    або першим спостереженим закриттям позиції, що існувала до історії
    (start_known=False → епізод «невизначений»); закінчується ФАКТИЧНИМ
    нулем (залишок ≤ PROFILE_ZERO_TOL від позиції). Долив (open посеред
    циклу) або невидиме зростання перед закриттям — та сама позиція
    (n_add_in), не новий цикл. Після досягнення цілі 95% (епізод
    завершено) відкриття, що виводить позицію вище решти, — нова позиція
    (from_dust): підтверджена відкриттям, а не близькістю до нуля."""
    lifes, cur = [], None
    for tx in txs:
        if tx["kind"] == "open":
            if cur is None or cur["zero"]:
                # v2.20 (аудит v2.19 №7): відкриття доводить ПОЧАТОК позиції лише
                # при нульовій попередній позиції (startPosition ≈ 0); перше
                # видиме відкриття з ненульовим startPosition — ДОЛИВ до
                # позиції, старшої за історію: початок невідомий (uncertain),
                # не «швидкий» — обрізана історія не робить повільний гаманець
                # підтверджено швидким
                _prev = float(tx.get("sp") or 0.0)
                _known = _prev <= max(1e-9, PROFILE_ZERO_TOL * (_prev + float(tx["sz"])))
                cur = {"start_known": bool(_known), "start_t": (tx["t"] if _known else None),
                       "opens": [tx], "closes": [], "pos": tx["sp"] + tx["sz"], "zero": False,
                       "n_add_in": (0 if _known else 1), "from_dust": 0}
                lifes.append(cur)
            elif cur.get("ep_end_t") is not None and tx["sp"] <= cur.get("dust_thr", 0.0):
                cur = {"start_known": True, "start_t": tx["t"], "opens": [tx], "closes": [],
                       "pos": tx["sp"] + tx["sz"], "zero": False, "n_add_in": 0, "from_dust": 1}
                lifes.append(cur)
            else:
                cur["n_add_in"] += 1
                cur["opens"].append(tx)
        ...
        # close
        if cur is None or cur["zero"]:
            cur = {"start_known": False, "start_t": None, "opens": [], "closes": [],
                   "pos": tx["sp"], "zero": False, "n_add_in": 0, "from_dust": 0}
            lifes.append(cur)
        elif tx["sp"] > cur["pos"] * (1.0 + 0.01) + 1e-9:
            cur["n_add_in"] += 1            # позиція виросла без видимого відкриття — той самий цикл
        cur["closes"].append(tx)
        rem = max(0.0, tx["sp"] - tx["sz"])
        cur["pos"] = rem
        if rem <= max(1e-9, tx["sp"] * PROFILE_ZERO_TOL):
            cur["zero"] = True
            cur["zero_t"] = tx["t"]
        # ціль епізоду (95%) — для from_dust (див. _profile_episode)
        if cur.get("sp0") and cur.get("ep_end_t") is None and rem <= cur["dust_thr"]:
            cur["ep_end_t"] = tx["t"]
        if cur.get("sp0") is None:
            # сигнал = перша агресивна tx ≥5% позиції ≥$100k — фіксуємо ціль
            if tx["aggr"] and tx["sp"] > 0 and tx["sz"] >= tx["sp"] * F4_CHUNK_PCT \
                    and tx["sp"] * tx["px"] >= F4_MIN_NOTIONAL:
                cur["sp0"] = tx["sp"]
                cur["dust_thr"] = tx["sp"] * (1.0 - F4_FULL_PCT)
                if rem <= cur["dust_thr"]:
                    cur["ep_end_t"] = tx["t"]
    return lifes

def _profile_episode(life, coin, now_ms, depth_fn=None):
    """Епізод розвантаження одного життєвого циклу (не більше одного на
    цикл): сигнал = перша агресивна tx ≥5% позиції ≥$100k; кінець = перша
    tx, після якої залишок ≤5% позиції на сигналі (усі закриття — мейкер,
    TWAP, ліквідація; долив не рве). Клас: fast (≤300 с), slow, inprog
    (сигнал <5 хв тому, не закрито), uncertain (початок циклу поза
    історією І видима частина швидка — аудит v2.18 №3: не «повільний»
    автоматично, а окрема категорія). Плюс придатність до follow (№6/№7):
    first_pct — частка сигнальної tx; after_n / after_pct / after_dur_s —
    агресивні закриття ПІСЛЯ доступного входу (сигнал + 5 с); maker_finish;
    one_shot; pre_signal_pct і dur_reduce_s — від першого скорочення
    позиції (аудит №7: дві тривалості). None — циклу без сигналу."""
    closes = life["closes"]
    sig_i = None
    for i, tx in enumerate(closes):
        if tx["aggr"] and tx["sp"] > 0 and tx["sz"] >= tx["sp"] * F4_CHUNK_PCT \
                and tx["sp"] * tx["px"] >= F4_MIN_NOTIONAL:
            sig_i = i
            break
    ...
    t0, sp0 = sig["t"], sig["sp"]
    usd0 = sp0 * sig["px"]
    dust = sp0 * (1.0 - F4_FULL_PCT)
    end_i = None
    for i in range(sig_i, len(closes)):
        if max(0.0, closes[i]["sp"] - closes[i]["sz"]) <= dust:
            end_i = i
            break
    end_t = closes[end_i]["t"] if end_i is not None else None
    win = closes[sig_i:(end_i + 1 if end_i is not None else len(closes))]
    if end_i is None:
        win = [c for c in win if c["t"] - t0 <= F4_MAX_UNLOAD_S * 1000]
    aggr_ts = [c["t"] for c in win if c["aggr"]]
    gaps = [(aggr_ts[i] - aggr_ts[i - 1]) / 1000.0 for i in range(1, len(aggr_ts))]
    after = [c for c in win if c["aggr"] and c["t"] > t0 + PROFILE_ENTRY_LAT_MS]
    after_pct = sum(c["sz"] for c in after) / sp0 if sp0 else 0.0
    after_dur = ((max(c["t"] for c in after) - t0) / 1000.0) if after else 0.0
    first_reduce = closes[0]
    pre_signal_pct = ((first_reduce["sp"] - sp0) / first_reduce["sp"]
                      if first_reduce["sp"] > sp0 > 0 else 0.0)
    d = (depth_fn(coin, sig["side"]) or 0) if depth_fn else 0
    ep = {"t0": t0, "end_t": end_t, "sp0": sp0, "usd": usd0,
          "ratio": (usd0 / d if d > 0 else None),
          "first_pct": sig["sz"] / sp0 if sp0 else 0.0,
          "after_n": len(after), "after_pct": after_pct, "after_dur_s": after_dur,
          "maker_finish": int(end_i is not None and not closes[end_i]["aggr"]),
          "one_shot": int(sp0 > 0 and sig["sz"] >= sp0 * F4_FULL_PCT),
          "pre_signal_pct": pre_signal_pct,
          "dur_reduce_s": ((end_t - first_reduce["t"]) / 1000.0 if end_t is not None else None),
          "gaps": gaps, "n_add_in": life["n_add_in"], "start_known": bool(life["start_known"]),
          "from_dust": life.get("from_dust", 0), "side": sig["side"]}
    if end_t is None:
        if now_ms - t0 < F4_MAX_UNLOAD_S * 1000:
            ep.update(cls="inprog", unload_s=None)
            return ep
        ep.update(cls="slow", unload_s=None)
        return ep
    unload = (end_t - t0) / 1000.0
    ep["unload_s"] = unload
    if unload > F4_MAX_UNLOAD_S:
        ep["cls"] = "slow"
    elif not life["start_known"]:
        ep["cls"] = "uncertain"     # видима частина швидка, початок циклу невідомий
    else:
        ep["cls"] = "fast"
    return ep
...
def _build_profile(fills, now_ms=None, depth_fn=None):
    ...
    v10 після аудиту пошуку швидких гаманців, 12.09):
    (а) філи → транзакції по hash (_profile_txs), ВІДКРИТТЯ теж читаються;
    (б) життєві цикли позиції (_profile_lifecycles): нова позиція лише через
        відкриття з нуля / від пилу після цілі; долив і невидиме зростання —
        той самий цикл; фактичний нуль ≠ ціль 95% (аудит №2/№4);
    (в) ≤1 епізод на цикл (_profile_episode) з класом fast/slow/uncertain/
        inprog і метриками придатності до follow (№6/№7);
    (г) ratio до ПОТОЧНОЇ глибини Binance у кваліфікації НЕ бере участі
        (№8: поведінка не залежить від сьогоднішньої ліквідності; гейт
        ratio — на момент кожного сигналу у follow_on_txs), лише
        інформативно; nr-гілка = той самий профіль (сумісність F7/F9);
    (д) кваліфікація (№3/№5, точні лічильники): ok (підтверджено) —
        n_fast ≥5 І 100·n_fast ≥ 70·(fast+slow+uncertain) — невизначені
        рахуються як повільні; status "uncertain" — не підтверджено, але
        якби невизначені були швидкими, пройшло б; інакше "no";
    (е) avg_gap_s — середня по ЕПІЗОДАХ (медіана пауз кожного) серед усіх
    ...
    for coin, txs in by_coin.items():
        for li, life in enumerate(_profile_lifecycles(txs)):
            n_life += 1
            ep = _profile_episode(life, coin, now_ms, depth_fn)
            if ep is None:
                n_nosig += 1
                continue
            ep["coin"] = coin
            ep["episode_id"] = f"{coin}:{li}"
            eps.append(ep)
    n_fast = sum(1 for e in eps if e["cls"] == "fast")
    n_slow = sum(1 for e in eps if e["cls"] == "slow")
    n_unc = sum(1 for e in eps if e["cls"] == "uncertain")
    n_inprog = sum(1 for e in eps if e["cls"] == "inprog")
    n_big = n_fast + n_slow + n_unc
    ok = bool(n_fast >= F4_MIN_EPISODES and n_big
              and 100.0 * n_fast >= F4_MIN_FAST_PCT * n_big)
    opt_ok = bool((n_fast + n_unc) >= F4_MIN_EPISODES and n_big
                  and 100.0 * (n_fast + n_unc) >= F4_MIN_FAST_PCT * n_big)
    status = "ok" if ok else ("uncertain" if opt_ok else "no")
    done = [e for e in eps if e["cls"] in ("fast", "slow", "uncertain")]
    fast_eps = [e for e in eps if e["cls"] == "fast"]
    unl_fast = [e["unload_s"] for e in fast_eps if e["unload_s"] is not None]
    unl_all = [e["unload_s"] for e in done if e["unload_s"] is not None]
    ep_gaps = [_median(e["gaps"]) for e in done if e["gaps"]]
    avg = (sum(ep_gaps) / len(ep_gaps)) if ep_gaps else 30.0
    def _pct(sel):
        return round(100.0 * sum(1 for e in done if sel(e)) / len(done), 1) if done else None
    def _med(key):
        v = [e[key] for e in done if e.get(key) is not None]
        return round(_median(v), 3) if v else None
    out = {"ok": ok, "status": status, "n_ep": n_fast, "n_big": n_big, "n_fast": n_fast,
           "n_slow": n_slow, "n_uncertain": n_unc, "n_inprog": n_inprog,
           "n_nosignal": n_nosig, "n_lifecycles": n_life, "n_nodepth": 0,
           "fast_pct": (round(100.0 * n_fast / n_big, 1) if n_big else None),
           "fast_lb95": (round(100.0 * _wilson_lb(n_fast, n_big), 1) if n_big else None),
           "unload_med_s": (round(_median(unl_fast), 1) if unl_fast else None),
           "unload_mean_s": (round(sum(unl_fast) / len(unl_fast), 1) if unl_fast else None),
           "unload_all_med_s": (round(_median(unl_all), 1) if unl_all else None),
           "avg_gap_s": round(avg, 1),
           # придатність до follow (аудит №6/№7)
           "one_shot_pct": _pct(lambda e: e["one_shot"]),
           "cont_pct": _pct(lambda e: e["after_n"] >= 1),
           "maker_finish_pct": _pct(lambda e: e["maker_finish"]),
           "after_pct_med": _med("after_pct"), "after_dur_med_s": _med("after_dur_s"),
           "pre_signal_pct_med": _med("pre_signal_pct"), "dur_reduce_med_s": _med("dur_reduce_s"),
           "episodes": [[e["episode_id"], e["cls"], e["t0"], e["end_t"], round(e["sp0"], 6),
                         round(e["usd"], 0), (round(e["unload_s"], 1) if e["unload_s"] is not None else None),
                         round(e["first_pct"], 4), e["after_n"], round(e["after_pct"], 4),
                         round(e["after_dur_s"], 1), e["maker_finish"], e["one_shot"],
                         round(e["pre_signal_pct"], 4),
                         (round(e["dur_reduce_s"], 1) if e["dur_reduce_s"] is not None else None),
                         e["n_add_in"], int(e["start_known"]), e["from_dust"]] for e in eps],
           "episodes_cols": ["episode_id", "cls", "t0_ms", "end_ms", "sp0", "usd", "unload_s", "first_pct",
                             "after_n", "after_pct", "after_dur_s", "maker_finish", "one_shot",
                             "pre_signal_pct", "dur_reduce_s", "n_add_in", "start_known", "from_dust"]}
    out["nr"] = {k: v for k, v in out.items() if k not in ("episodes", "episodes_cols")}
    if bad:
        out.update(ok=False, status="err", err=1, bad_rows=bad)
        out["nr"]["ok"] = False
        out["nr"]["status"] = "err"
    return out
...
def _fetch_profile_locked(addr):
    ...
    with strat2_lock:
        _old = wallet_profiles.get(addr) or {}
        # v2.18: статус за попередньою версією алгоритму — для трьох груп
        # після перерахунку (зберіг допуск / став невизначеним / втратив)
        if _old.get("v") != PROFILE_ALGO_V:
            prof["prev_v"] = _old.get("v")
            prof["prev_status"] = (_old.get("status") or ("ok" if _old.get("ok") else
                                   ("err" if _old.get("err") else "no"))) if _old else None
        else:
            prof["prev_v"] = _old.get("prev_v"); prof["prev_status"] = _old.get("prev_status")
        wallet_profiles[addr] = prof
        profiles_fetching.discard(addr)
    # v2.18 (пріоритет 4, відтворюваність): сирі філи останнього запиту —
    # окремим gz-файлом на гаманець (перезаписується), таблиця епізодів —
    # у профілі (episodes); версія, межі й момент розрахунку — там само
    try:
        if not prof.get("err"):
            import gzip
            _rd = os.path.join(DATA_DIR, "profiles_raw")
            os.makedirs(_rd, exist_ok=True)
            _tmp = os.path.join(_rd, f"{addr}.json.gz.tmp")
            with gzip.open(_tmp, "wt", encoding="utf-8") as _gf:
                json.dump({"addr": addr, "fetched": prof["fetched"], "v": PROFILE_ALGO_V,
                           "window_start_ms": int(now_ms - PROFILE_WINDOW_D * 86400 * 1000),
                           "end_ms": int(now_ms), "n": len(fills), "fills": fills}, _gf)
            # v2.20 (аудит v2.19, залишкове): сирі філи НЕ перезаписуються
            # безслідно — ротація трьох поколінь (.json.gz → .1 → .2): кожен
            # із останніх профілів відтворюваний зі своїх точних вихідних даних
            _cur = os.path.join(_rd, f"{addr}.json.gz")
            for _g in (2, 1):
                _src = _cur if _g == 1 else os.path.join(_rd, f"{addr}.{_g - 1}.json.gz")
                if os.path.exists(_src):
                    os.replace(_src, os.path.join(_rd, f"{addr}.{_g}.json.gz"))
            os.replace(_tmp, _cur)
    except Exception as _re:
        stats["profile_raw_err"] = stats.get("profile_raw_err", 0) + 1
        if stats["profile_raw_err"] in (1, 10):
            print(f"  [F4] сирі філи {addr[:10]}… не записано: {_re}")
    try:
    ...
    print(f"  [F4] профіль {addr[:10]}…: {prof.get('status', 'ok' if prof['ok'] else 'no')} "
          f"швидких={prof.get('n_fast', 0)} повільних={prof.get('n_slow', 0)} "
          f"невизн.={prof.get('n_uncertain', 0)}"
...
def _strat2_tick():
    ...
    with strat2_lock:
        for pid, p in list(rev_open.items()):
            if p.get("done"):
                ...
                continue
            px = _tr_px_now(p, 30.0)   # v2.20: ціна з біржі виконання трекера
            if p["state"] == "armed":
    ...
    if rev_price or fol_exit:
        ...
        _pass_t0 = time.time()
        _fetched = set()   # (coin, бік, біржа) — реальні запити стакану за прохід
        def _ppx(coin, bside, strict=False, qty=None, exch=None, min_recv_ms=None):
            ...
            # v2.19: ключ кешу — і кількість (вихід на контракти входу), і біржа;
            # кап рахує ЗАПИТИ (монета, бік, біржа): різні кількості одного
            # стакану проходяться без нового запиту (_raw_book_cache)
            k = (coin, bside, strict, exch or EXEC_EXCH, round(qty, 8) if qty else None)
            fk = (coin, bside, exch or EXEC_EXCH)
            # v2.20 (аудит №1): кешована відповідь придатна лише якщо ціну
            # ОТРИМАНО не раніше за рішення цього запитувача (min_recv_ms)
            if k in _pxc and min_recv_ms:
                _rm = ((_pxc[k][2] or {}).get("recv_ms") or 0)
                if _rm and _rm < min_recv_ms:
                    _pxc.pop(k, None)
            if k not in _pxc:
                if (fk not in _fetched and len(_fetched) >= 6) or time.time() - _pass_t0 > 12.0:
                    # кап на прохід: решта — з кеш-міда ТІЄЇ Ж біржі (тик не має
                    # тривати хвилину, інші трекери втрачають семпли);
                    # для входу мід не годиться → no_book; семпл до рішення — ні
                    _mid, _age, _mts = _mid_age_exch(exch or EXEC_EXCH, coin)
                    _m = {"mid": _mid, "px_age_ms": _age,
                          "recv_ms": (int(_mts * 1000) if _mts else int(time.time() * 1000)),
                          "exch": exch or EXEC_EXCH}
                    _pre = bool(min_recv_ms and _mts and _mts * 1000.0 < float(min_recv_ms))
                    if strict:
                        _pxc[k] = (None, "no_book", _m)
                    else:
                        _pxc[k] = ((_mid if (_mid and (_age or 0) <= 35_000 and not _pre) else None), "mid", _m)
                else:
                    _fetched.add(fk)
                    _pxc[k] = _paper_px(coin, bside, max_mid_age_ms=35_000, strict=strict,
                                        qty=qty, exch=exch, min_recv_ms=min_recv_ms)
            return _pxc[k]
        for pid, what in rev_price:
            with strat2_lock:
                ...
                _q, _ex = p.get("entry_qty"), _tr_exch(p)
            if not req: continue
            ...
            _mr = int(round(float(req.get("ts") or 0) * 1000)) or None   # рішення фази 1
            xpx, xsrc, xmeta = _ppx(coin, bside, strict=(what == "arm"),
                                    qty=(None if what == "arm" else _q), exch=_ex, min_recv_ms=_mr)
            if what == "arm" and xpx and xsrc == "book":
                # v2.19: поля виконання входу (фандинг — мережа) ПОЗА локом
                xmeta = dict(xmeta); xmeta["_xf"] = _entry_exec_fields(xpx, xmeta, coin)
        ...
        for fid in fol_exit:
            with strat2_lock:
                ...
                _q, _ex = p.get("entry_qty"), _tr_exch(p)
                _mr = int(round(float((p.get("exit_pending") or {}).get("ts") or 0) * 1000)) or None
            xpx, xsrc, xmeta = _ppx(coin, "BUY" if side == "SHORT" else "SELL", qty=_q, exch=_ex,
                                    min_recv_ms=_mr)
            fpriced.append((fid, xpx, xsrc, xmeta))
        def _apply_delay(xmeta, now2, req=None):
            """аудит v2.17 I: ціна застосовується з моментом ОТРИМАННЯ; якщо
            від отримання до застосування минуло > BOOK_APPLY_MAX_S — це вже
            не «зараз», повторний запит наступним тиком. v2.20 (аудит v2.19 №1):
            ціна, ОТРИМАНА до моменту рішення (req.ts фази 1), — не «після
            сигналу» (захист у глибину до min_recv_ms). → (recv_s, delay_s)
            або (None, None) якщо застаріла / до рішення."""
            recv = (xmeta or {}).get("recv_ms")
            recv_s = (recv / 1000.0) if recv else now2
            delay = max(0.0, now2 - recv_s)
            if delay > BOOK_APPLY_MAX_S:
                stats["book_apply_stale"] = stats.get("book_apply_stale", 0) + 1
                return None, None
            _rt = float((req or {}).get("ts") or 0)
            if recv and _rt and recv_s < _rt - 1e-6:
                stats["book_pre_decision"] = stats.get("book_pre_decision", 0) + 1
                return None, None
            return recv_s, delay
        ...
        with strat2_lock:
            for pid, what, xpx, xsrc, xmeta in priced:
                ...
                if what == "arm":
                    ...
                    if recv_s is None:
                        # ціна застаріла до застосування — запит НЕ відновлюємо
                        # (рев'ю v2.19 №2: відновлений arm_hit зі старим ts блокував
                        # повтор на 30 с); фаза 1 створить новий наступним тиком
                        continue
                    base_px = xpx
                    # консервативно: гірша з цін (стакан / тригер)
                    p["entry_px"] = (max(base_px, trig) if p["side"] == "LONG"
                                     else min(base_px, trig))
                    p["entry_ts"] = recv_s   # аудит I: момент отримання ціни
                    p["decision_ms"] = int(round(float(req.get("ts") or 0) * 1000))   # v2.20: рішення фази 1
                    p["entry_src"] = xsrc
                    p["entry_px_mid"] = xmeta.get("mid") or mid
                    p["px_age_ms"] = xmeta.get("px_age_ms")
                    p.update(xmeta.get("_xf") or {"exch": xmeta.get("exch") or EXEC_EXCH})
                    if p.get("entry_qty"):
                        p["entry_qty"] = SIM_POSITION_USD / p["entry_px"]   # від ціни ВХОДУ (гіршої)
                    p["state"] = "open"
                    rev_exited = True
                    ...
                    if req["kind"] == "twap":
                        ...
                        if isinstance(tw, dict):
                            tw["exit_px"] = base_px
                            tw["exit_src"] = xsrc
                            tw["exit_ts_ms"] = int(round(recv_s * 1000))   # аудит I
                            p["exec_delay_s"] = round(_dl, 3)
                            p["exit_decision_ms"] = int(round(float(req.get("ts") or 0) * 1000))   # v2.20
                            p["exit_px_filled"] = xmeta.get("px_filled")
                            p["exit_fill_frac"] = (xmeta.get("fill_frac") if xmeta.get("fill_frac") is not None
                                                   else (1.0 if str(xsrc).startswith("book") else None))
                            p["exit_partial"] = int((p["exit_fill_frac"] or 1.0) < 1.0 - 1e-9)
                            p["funding_pct"] = _funding_pct(p, tw["exit_ts_ms"])
                        p["trade_closed_ts"] = p.get("trade_closed_ts") or recv_s
                    ...
                    p["exit_px"] = base_px
                    p["exit_src"] = xsrc
                    p["exit_ts_ms"] = int(round(recv_s * 1000))   # аудит I: момент отримання ціни
                    p["exec_delay_s"] = round(_dl, 3)
                    p["exit_decision_ms"] = int(round(float(req.get("ts") or 0) * 1000))   # v2.20
                    p["exit_px_filled"] = xmeta.get("px_filled")
                    p["exit_fill_frac"] = (xmeta.get("fill_frac") if xmeta.get("fill_frac") is not None
                    ...
                    p["exit_partial"] = int((p["exit_fill_frac"] or 1.0) < 1.0 - 1e-9)
                    p["funding_pct"] = _funding_pct(p, p["exit_ts_ms"])   # v2.19
                    p["exit_min"] = req["min"]
            ...
            for fid, xpx, xsrc, xmeta in fpriced:
                ...
                recv_s, _dl = _apply_delay(xmeta, now2, req)
                if recv_s is None:
                    p["exit_pending"] = req; continue   # застаріла до застосування — повтор
                now2 = recv_s   # аудит I: час виходу = момент отримання ціни
                p["exec_delay_s"] = round(_dl, 3)
                p["exit_decision_ms"] = int(round(float(req.get("ts") or 0) * 1000))   # v2.20
                p["exit_px_filled"] = xmeta.get("px_filled")
                p["exit_fill_frac"] = (xmeta.get("fill_frac") if xmeta.get("fill_frac") is not None
                ...
                p["exit_partial"] = int((p["exit_fill_frac"] or 1.0) < 1.0 - 1e-9)
                p["funding_pct"] = _funding_pct(p, int(round(now2 * 1000)))   # v2.19
                reason = req["reason"]
...
def _load_settlements():
    ...
    try:
        ...
        if cur is not None:
            # v2.20 (аудит v2.19 №4): придатний ЛИШЕ рядок ПОТОЧНОЇ версії
            # розрахунку — старіші рахувались на кешах без доказу повноти
            # (обрізані сторінки v2.17) / без прив'язки до тригера / без
            # фандингу у кривій; до перерахунку угода показується як
            # нерозрахована (pending), не з хибним «verified»
            def _sv(r):
                try:
                    return int(str(r.get("settle_v") or "0").strip())
                except ValueError:
                    return 0
            old = [k for k, r in data.items() if (r.get("settle_v") or "").strip() != str(cur)]
            stats["settle_old_v"] = sum(1 for k in old if _sv(data[k]) < 3)
            for k in old:
                data.pop(k, None)
            stats["settle_prev_v"] = len(old)
    except Exception as e:
...
def _sig_check_rev(st, srow, fnum):
    """v2.19 (аудит v2.17 A/B): ВАЛІДНІСТЬ СИГНАЛУ реверсу за settlement —
    окремо від якості ціни. Правило входу перевіряється ще раз по ВСІХ
    філах кита (live бачив лише тейкерські, лише в межах батча і за 60 с):
    повне закриття, епізод <5 хв (когорта 1..5), рух епізоду ≥ поріг
    стратегії, лаг ≤60 с; R7 — найбільший філ ≥95% і ≥$100k; R2 — тригер
    на стрічці. → (1 | 0 | None, причина); None = перевірити нема чим
    (settlement ще нема / філів кита не отримано / історія насичена)."""
    if not srow:
        return None, "pending"
    fl = srow.get("flags") or ""
    if "no_fills" in fl or "hl_sat" in fl:
        return None, "no_fills"
    if "no_whale_fill" in fl:
        return 0, "no_whale_fill"
    if "full_close" not in fl:
        return 0, ("partial_close" if "partial_close" in fl else "close_unknown")
    b_ = fnum(srow.get("dump_bucket")); mv = fnum(srow.get("dump_move_pct"))
    if b_ is None or mv is None:
        return None, "no_episode"
    if not (1 <= b_ <= 5):
        return 0, "long_episode"
    if mv + 1e-9 < _REV_THR.get(st, 1.0):
        return 0, "small_move"
    lag = fnum(srow.get("lag_s"))
    if lag is None or lag > REV_MAX_FILL_AGE_S:
        return 0, "stale_fill"
    if lag < 0:
        return 0, "neg_lag"        # v2.20 (аудит №1): ціна до філа кита — не сигнал
    if st == R7_NAME:
        mp, mu = fnum(srow.get("whale_max_fill_pct")), fnum(srow.get("whale_max_fill_usd"))
        if mp is None or mp < F4_FULL_PCT or (mu or 0) < R7_MIN_TX_USD:
            return 0, "not_one_shot"
    if st == "R2_breakout" and "no_trigger" in fl:
        return 0, "no_trigger"
    return 1, ""

def _sig_check_fol(srow, fnum):
    """v2.19: валідність follow-сигналу за settlement — філ кита знайдено
    і лаг виконання ≤20 с (те саме правило, що гейт входу; для старих
    рядків — єдиний доказ). → (1 | 0 | None, причина)."""
    if not srow:
        return None, "pending"
    fl = srow.get("flags") or ""
    if "no_fills" in fl or "hl_sat" in fl:
    ...
    if "no_whale_fill" in fl:
        return 0, "no_whale_fill"
    if "trigger_unmatched" in fl:
        # v2.20 (аудит №3): settlement НЕ знайшов записану тригерну транзакцію
        # серед філів кита — підставляти «останній підхожий філ» не можна:
        # перевірка неповна (None), не дозвіл
        return None, "trig_unmatched"
    lag = fnum(srow.get("lag_s"))
    if lag is None or lag > FOLLOW_MAX_FILL_AGE_S:
        return 0, "stale_fill"
    if lag < 0:
        return 0, "neg_lag"        # v2.20 (аудит №1): нижня межа лага
    return 1, ""

def _exit_partial_of(r, fnum):
    """v2.20 (аудит v2.19 №5): частковість виходу — і за новою колонкою
    exit_fill_frac (<1), і за СТАРИМ явним джерелом exit_src="book_partial"
    (рядки до 2.19 колонки не мають — «видимі $100 на $1000» не стають
    повним виходом від оновлення). Частковий вхід (entry_src book_partial у
    старих рядках) — теж не повне виконання."""
    ff = fnum(r.get("exit_fill_frac"))
    if ff is not None and ff < 1.0 - 1e-9:
        return 1
    if str(r.get("exit_src") or "").startswith("book_partial"):
        return 1
    if str(r.get("entry_src") or "").startswith("book_partial"):
        return 1
    return 0

def _px_check(vstat, exit_partial):
    """Якість перевірки ціни: 1 — verified (обидві оцінки, вихід повний),
    0 — не підтверджено (live_only / tape_only / none / частковий вихід),
    None — pending (settlement ще нема)."""
    if vstat == "pending":
        return None
    return int(vstat == "verified" and not exit_partial)

HEAD_STRICT = True   # v2.19: заголовок лише з підтвердженим сигналом і перевіреною ціною
                     # (тестові сюїти старої семантики вимикають; у проді — завжди True)

def _head_ok(t):
    """v2.19 (аудит A): ЗАГОЛОВКОВА вибірка гіпотези = увійшли, є офіційний
    net, вихід не пізній, профіль не «невизначений», сигнал ПІДТВЕРДЖЕНО
    settlement (sig_ok=1), ціна ПЕРЕВІРЕНА (px_ok=1: обидві оцінки, вихід
    повний), правило виходу виконане (exit_ok≠0). Усі інші paper-угоди
    лишаються у журналі виконання (paper), не в оцінці гіпотези."""
    if t.get("entered", 1) == 0 or t.get("net30") is None:
        return False
    if t.get("late") or t.get("stale") or t.get("unc"):
        return False
    if not HEAD_STRICT:
        # нестрогий режим (лише тестові сюїти до 2.19): старий допущений рядок —
        # лише з підтвердженим сигналом, як у v2.19 (_rev_adm_legacy)
        return t.get("exit_ok", 1) != 0 and (not t.get("adm") or t.get("sig_ok") == 1)
    if t.get("sig_ok", 1) != 1:
        return False
    px_ok = t.get("px_ok")
    if px_ok is None and "px_ok" not in t:
        px_ok = 1 if t.get("status") == "verified" else 0
    if px_ok != 1:
        return False
    # правило виходу не перевірене (R8 без реплею — exit_ok None) — теж поза заголовком
    return t.get("exit_ok", 1) == 1

def _group_stats(trs_, keyf):
    """v2.18 (аудит №9): результат по ГРУПАХ (гаманці / дні / монети) —
    медіана групових медіан і частка найбільшої групи в n, щоб один кит,
    один день чи одна монета не домінували у заголовку. v2.20 (аудит v2.19
    №10): ТОЙ САМИЙ допуск, що й заголовок (_head_ok) — раніше групи брали
    рядки з непідтвердженим сигналом/ціною."""
    g = {}
    for t in trs_:
        if not _head_ok(t):
            continue
        g.setdefault(keyf(t), []).append(t["net30"])
    if not g:
        return {"n_groups": 0, "median_of_medians": None, "top_share": None, "top_key": None}
    meds = [_median(v) for v in g.values()]
    total = sum(len(v) for v in g.values())
    top = max(g.items(), key=lambda kv: len(kv[1]))
    return {"n_groups": len(g), "median_of_medians": _median(meds),
            "top_share": round(100.0 * len(top[1]) / total, 1), "top_key": str(top[0] or "")}

def _eval_ts():
    try:
        return time.mktime(time.strptime(EVAL_SINCE, "%Y-%m-%d"))
...
def _agg_block(trs, now, H, buckets=False):
    ...
    Заголовкова вибірка (_head_ok, v2.19): офіційний net угод з
    ПІДТВЕРДЖЕНИМ сигналом і ПЕРЕВІРЕНОЮ ціною, без пізніх виходів (late),
    старих F без доказу свіжості (stale), «невизначених» профілів (unc).
    Окремо — журнал усіх paper-угод (paper): фактичні входи й їхні втрати
    не видаляються, щоб очистити гіпотезу."""
    head = [t for t in trs if _head_ok(t)]
    nets = [t["net30"] for t in head]
    paper_all = [t["net30"] for t in trs if t.get("entered", 1) != 0 and t.get("net30") is not None]
    ...
    out = {"n": len(nets), "median": _median(nets),
           ...
           # v2.18: угоди за профілем зі статусом «невизначений» — окремо
           "n_prof_unc": sum(1 for t in trs if t.get("unc")),
           "prof_unc": _stat_small([t["net30"] for t in trs if t.get("unc") and t.get("net30") is not None
                                    and not t.get("late")]),
           # v2.19 (аудит A/B/E/J): три факти окремо — сигнал (settlement),
           ...
           "n_px_pending": sum(1 for t in trs if t.get("entered", 1) != 0 and t.get("net30") is not None
                               and t.get("px_ok", 1) is None),
           "n_exit_partial": sum(1 for t in trs if t.get("exit_partial")),
           "n_exit_invalid": sum(1 for t in trs if t.get("exit_ok") == 0),
           ...
           # аудит J: n_verified — той самий знаменник, що й median verified (заголовок)
           "n_verified": len(ver),
           ...
           # v2.20 (аудит v2.19, рекомендація 7): частка перевірених серед усіх
           # paper-угод — картка показує поряд «уся поведінка» і «доведена гіпотеза»
           "pct_verified": (round(100.0 * len(nets) / len(paper_all), 1) if paper_all else None),
           # рекомендація 6: оцінювальний період після фіксації правил — окремо
           "eval_since": EVAL_SINCE,
           "eval": _stat_small([v for ts, v in items if ts is not None and _ev_ts is not None and ts >= _ev_ts]),
           "eval_paper": _stat_small([t["net30"] for t in trs if t.get("entered", 1) != 0
                                      and t.get("net30") is not None and t.get("ts") is not None
                                      and _ev_ts is not None and t["ts"] >= _ev_ts]),
           # концентрація заголовка (той самий допуск): гаманець / день / монета
           "by_wallet": _group_stats(trs, lambda t: t.get("wallet")),
           "by_day": _group_stats(trs, lambda t: (t.get("date") or "")[:10]),
           "by_coin": _group_stats(trs, lambda t: t.get("coin"))}
...
def strat2_api():
    ...
    def _rev_adm_legacy(st):
        # v2.19 (аудит A/B): старі рядки (до STRAT_SINCE, ≥2.10) ідуть у той
        # самий цикл, що й поточні, — валідність сигналу (_sig_check_rev) і
        # якість ціни (verified: обидві оцінки зі стрічки) перевіряються
        # ОДНАКОВО для всіх; кандидат — лише рядок, що має settlement і
        # підтверджений сигнал; у заголовок він потрапить лише з перевіреною
        # ціною (_head_ok) — «старий live-only PnL» більше не допускається
        # v2.20 (аудит v2.19 №10): у ЖУРНАЛ paper ідуть УСІ старі рядки (≥2.10) —
        # і зі спростованим/відсутнім settlement (їхні втрати не зникають);
        # у заголовок — лише через _head_ok (сигнал + ціна). Кандидати
        # (settlement є, сигнал підтверджено) рахуються окремо
        adm, cand = [], 0
        for r in rev_all:
            ...
            if s_ and _sig_check_rev(st, s_, fnum)[0] == 1:
                cand += 1
        return adm, cand
    ...
    for st in ("R1_загальний", "R2_breakout", "R3_великі", "R4_великий",
        ...
        adm_rows, n_adm_cand = _rev_adm_legacy(st)
        ...
        for r in rows:
            ...
            # v2.19 (аудит A/E/F/J): три факти окремо
            sig_ok, sig_why = (_sig_check_rev(st, srow, fnum) if entered else (None, ""))
            _ff = fnum(r.get("exit_fill_frac"))
            exit_partial = _exit_partial_of(r, fnum)   # v2.20: і старий book_partial
            # px_ok: без settlement — pending (None); є settlement — verified І повний вихід
            px_ok = ((_px_check(vstat, exit_partial) if srow else None) if entered else None)
            exit_ok, exit_why = (1, "")
            if entered:
                # no_price — збій ЦІНИ live (px_ok/стрічка), не порушення правила виходу
                if exit_reason == "timer_late":
                    exit_ok, exit_why = 0, "late"
                elif st == R8_NAME:
                    # аудит F: правило TP реплеєм по стрічці (settlement v4):
                    # live «tp» без перетину на стрічці — фантом; live «timer»,
                    # коли стрічка перетнула TP, — пропущений TP
                    _fl8 = (flags or "")
                    ...
                    if "no_dump" in _fl8 and _hit8 is None:
                        # рев'ю №6: R8 без tp_px і без дампу — правило нема з чим
            ...
            trades.append({"date": r.get("date"), "coin": r.get("coin"),
                ...
                # v2.19: сигнал / ціна / вихід — окремі факти; виконання
                "sig_ok": sig_ok, "sig_why": sig_why, "px_ok": px_ok,
                "exit_ok": exit_ok, "exit_why": exit_why, "exit_partial": exit_partial,
                "fill_frac": _ff, "funding": fnum(r.get("funding_pct")),
                "exch": r.get("exch") or "", "exec_delay": fnum(r.get("exec_delay_s")),
                "net_tp_mkt": (fnum(srow.get("net_tp_mkt_pct")) if srow else None),
                "net_tp_lim": (fnum(srow.get("net_tp_lim_pct")) if srow else None),
                "entered": entered, "ts": ts,
                "_ct": (_parse_curve(srow.get("curve_tape"), REV_TRACK_MIN) if (srow and entered) else None),
                "_cl": live_curve})
        # v2.19 (аудит J): закриті, але ще не записані R-трекери (вихід m30
        # вирішено, крива ведеться до m60) — pending-рядки у списку/журналі
        trades.extend(_pending_rev_rows(st, _csv_keys))
        ...
        blk.update({
            ...
            # допущені старі рядки — лише ті, що у заголовку (сигнал + ціна)
            "n_adm_legacy": sum(1 for t in trades if t.get("adm") and _head_ok(t)),
            "n_adm_cand": n_adm_cand,
        })
        # v2.20: усі старі R-рядки (≥2.10) показані у журналі paper — вони вже
        # не «приховані» (legacy_rows зменшено на len(adm_rows) вище)
        out["strategies"][st] = blk
    out["legacy_rows"] = legacy_rows   # v2.16/v2.20: мінус старі R-рядки, показані у журналі

    for st in list(FOLLOW_TIMERS) + [F6_NAME, F4_NAME, F5_NAME, F8_NAME,
                                     F7_NAME, F9_NAME, F10_NAME]:
        ...
        for r in rows:
            ...
            # v2.19 (аудит A/B): валідність сигналу за settlement для ВСІХ
            # рядків (старий без settlement — pending, не «допущений»);
            # stale = спростований лаг (сумісність UI)
            sig_ok, sig_why = _sig_check_fol(srow, fnum)
            # stale (сумісність v2.17): СТАРИЙ рядок (до DATA_ALGO_V) без доказу
            # свіжості за settlement; для поточних рядків доказ — sig_ok
            stale = 0
            ...
            _ff = fnum(r.get("exit_fill_frac"))
            exit_partial = _exit_partial_of(r, fnum)   # v2.20: і старий book_partial
            px_ok = _px_check(vstat, exit_partial) if srow else None   # без settlement — pending
            ts = _ts_local(r.get("date_open"))
            fs = fnum(r.get("first_shot"))
            if fs == 1: n_first += 1
            pf = {"n_fast": fnum(r.get("prof_n_fast")),
                  "n_slow": fnum(r.get("prof_n_slow")),
                  "n_unc": fnum(r.get("prof_n_uncertain")), "lb95": fnum(r.get("prof_fast_lb95")),
                  "cont": fnum(r.get("prof_cont_pct")), "oneshot": fnum(r.get("prof_one_shot_pct")),
            ...
            trades.append({"date": r.get("date_open"), "coin": r.get("coin"),
                ...
                # v2.19: сигнал / ціна / вихід — окремі факти; виконання
                "sig_ok": sig_ok, "sig_why": sig_why, "px_ok": px_ok,
                "exit_ok": (0 if late else 1), "exit_partial": exit_partial, "fill_frac": _ff,
                "funding": fnum(r.get("funding_pct")), "exch": r.get("exch") or "",
                "exec_delay": fnum(r.get("exec_delay_s")),
                ...
                "_cl": None})
        blk = _agg_block(trades, now, REV_TRACK_MIN)   # v2.20: групи всередині (_head_ok)
        ...
        blk.update({
            ...
            "prof": {"wallets": len(pw),
                     ...
                     # v2.18: невизначені епізоди, Вілсон LB, придатність до follow
                     "n_uncertain": sum(int(p.get("n_unc") or 0) for p in pw),
                     "lb95_med": _median([p["lb95"] for p in pw if p.get("lb95") is not None]),
                     "cont_pct_med": _median([p["cont"] for p in pw if p.get("cont") is not None]),
                     "one_shot_pct_med": _median([p["oneshot"] for p in pw if p.get("oneshot") is not None])},
        })
        out["strategies"][st] = blk
    ...
    def _tw_trades(rs):
        ...
        for r in rs:
            ...
            reason = r.get("exit_reason") or ""
            # рев'ю v2.19 №3: три факти й для TWAP-рядків — інакше _head_ok
            # мовчки падав на status=="verified", а _agg_block рахував кожен
            # нерозрахований рядок як «ціна не підтверджена». Сигнал TWAP =
            # заявка, звірена з біржею при вході (verify_ok) — підтверджений
            # за побудовою; ціна — settlement; правило виходу — не пізній
            _ff = fnum(r.get("exit_fill_frac"))
            _xpart = _exit_partial_of(r, fnum)   # v2.20: і старий book_partial
            _late = int(reason.endswith("_late"))
            out_.append({"date": r.get("date_entry"), "coin": r.get("coin"),
                ...
                "sig_ok": 1, "sig_why": "",
                "px_ok": (_px_check(vstat, _xpart) if srow else None),
                "exit_ok": (0 if _late else 1), "exit_why": ("late" if _late else ""),
                "exit_partial": _xpart, "fill_frac": _ff,
                "funding": fnum(r.get("funding_pct")), "exch": r.get("exch") or "",
                "exec_delay": fnum(r.get("exec_delay_s")),
                "no_price": int(reason == "no_price"),
                "late": _late, "stale": 0,   # аудит-2 №15
                "grace": None, "vault": 0,
                "entry_src": r.get("entry_src") or "", "exit_src": r.get("exit_src") or "",
                "peak": fnum(r.get("peak_pct")), "trough": fnum(r.get("trough_pct")),
    ...
    def _tw_block(rs, trs=None):
        ...
        # v2.20 (аудит v2.19 №10): «підтверджено завершені» — підмножина
        # ЗАГОЛОВКА (той самий допуск _head_ok: перевірена ціна), а не будь-який
        # рядок із completed=1 без settlement
        conf = [t["net30"] for t in trs
                if fnum(t.get("completed"), 0) == 1 and fnum(t.get("cancel_after"), 0) == 0
                and _head_ok(t)]
    ...
    with strat2_lock:
        ...
        def _pstat(v):
            if not isinstance(v, dict):
                return "unknown"
            if v.get("v") != PROFILE_ALGO_V:
                return "pending"
            if v.get("err"):
                return "err"
            if v.get("truncated") or now - v.get("fetched", 0) >= PROFILE_HARD_TTL_S:
                return "pending"   # діра в історії / застарів — як f4_ok у follow_on_txs
            return v.get("status") or ("ok" if v.get("ok") else "no")
        _pv = list(wallet_profiles.values())
        _st_cnt = {}
        for v in _pv:
            _st_cnt[_pstat(v)] = _st_cnt.get(_pstat(v), 0) + 1
        # v2.18 (аудит №3): три групи після перерахунку профілів за новою
        # версією — хто зберіг підтверджений допуск, хто став невизначеним,
        # хто перестав відповідати правилу; плюс хто отримав допуск уперше
        _mig = {"kept": 0, "downgraded": 0, "lost": 0, "gained": 0}
        for v in _pv:
            if not isinstance(v, dict) or v.get("v") != PROFILE_ALGO_V:
                continue
            was_ok = (v.get("prev_status") == "ok")
            st_ = _pstat(v)
            if was_ok and st_ == "ok": _mig["kept"] += 1
            elif was_ok and st_ == "uncertain": _mig["downgraded"] += 1
            elif was_ok: _mig["lost"] += 1
            elif st_ == "ok" and v.get("prev_status") not in (None, "ok"): _mig["gained"] += 1
        out["profiles"] = {"total": len(wallet_profiles),
                           "ok": _st_cnt.get("ok", 0),
                           "ok_nr": _st_cnt.get("ok", 0),          # сумісність UI (nr = основна з v2.18)
                           "uncertain": _st_cnt.get("uncertain", 0),
                           "no": _st_cnt.get("no", 0),
                           "pending": _st_cnt.get("pending", 0) + _st_cnt.get("unknown", 0),
                           "err": _st_cnt.get("err", 0),
                           "fetching": len(profiles_fetching),
                           "migration": _mig, "algo_v": PROFILE_ALGO_V}
    ...
    # аудит v2.17 J: «ПЕРЕВІРЕНО» — та сама заголовкова вибірка, що й картка
    # R1 (сигнал підтверджено, ціна verified), а не будь-який live_only
    _r1h = [t for t in _r1 if _head_ok(t)]
    def _mag_v(t):
        dm = t.get("dump_move")
        if dm is None:
            return None
    ...
    # рев'ю v2.19 №8: тіньовий рядок із запізнілою детекцією (скан-діф віддає
    # філи до години давності) міряє «після закриття» від чужої ціни —
    # поза спостереженням (нові такі рядки не створюються, старі — фільтр)
    def _lag_ok(r):
        lg = fnum(r.get("lag_s"))
        return lg is None or lg <= REV_MAX_FILL_AGE_S
    outs_cur = [r for r in outs if _v_ok(r.get("algo_v"), RESEARCH_SINCE) and _lag_ok(r)]
...
def _twap_enter(rec, now, px):
    ...
    decision_ms = int(now * 1000)   # v2.20: котирування не раніше за рішення
    epx, esrc, emeta = _paper_px(rec["coin"], "SELL" if our == "SHORT" else "BUY",
                                 strict=True, min_recv_ms=decision_ms)
    if not epx:
        ...
        return
    # аудит v2.17 I: час виконання = момент отримання ціни
    now = (emeta.get("recv_ms") / 1000.0) if emeta.get("recv_ms") else time.time()
    if rec.get("end") is not None and now > rec["end"] + TWAP_LATE_S:
        ...
        _twap_drop(rec, "late")
        return
    _xf = _entry_exec_fields(epx, emeta, rec["coin"])   # v2.19: до локу (мережа)
    opened, busy_c = [], []
    with strat2_lock:
        for thr in TWAP_COHORTS:
            if move + 1e-9 < thr:
            ...
            tr = {"sig_id": rec["id"], "strategy": strat, "state": "open",
                  ...
                  "px_age_ms": emeta.get("px_age_ms"), **_xf,
                  "decision_ms": decision_ms,
                  "detect_ms": (int(float(rec.get("created") or 0) * 1000) or None),   # момент реєстрації поста
...
def _twap_row(p, now=None, ex=None):
    ...
    return ([p["sig_id"], p["strategy"], _dt(p["entry_ts"]), p["coin"],
             ...
             (round(mx - costs_eff - fund, 4) if mx is not None else ""),
             round(costs, 4),
             (round(p["peak"], 4) if p["peak"] > -999 else ""),
             (round(p["trough"], 4) if p["trough"] < 999 else ""),
...
def main():
    ...
    threading.Thread(target=run_bn_px_poller,    daemon=True).start()   # v2.20: потік цін Binance
    ...
    threading.Thread(target=run_profile_refresher, daemon=True).start()   # v2.18
